...
int main() {
    ...
    for (int i = 0; i < (1 << 2 * n); i++) {
        ...
        for (int ii = 0; ii < n; ii++) {
            if ((i & (1ll << 2 * ii)) && (i & (1ll << 2 * ii + 1)))an += z[ii];
            if ((i & (1ll << 2 * ii)) && !(i & (1ll << 2 * ii + 1)))bn += z[ii];
            if (!(i & (1ll << 2 * ii)) && (i & (1ll << 2 * ii + 1)))cn += z[ii];
            if (!(i & (1ll << 2 * ii)) && !(i & (1ll << 2 * ii + 1)))ds++;
        }
        if (an * bn * cn == 0)continue;
        ans = min(abs(an-a) + abs(b-bn) + abs(cn-c) + (n - 3 - ds) * 10, ans);
    }
    cout << ans << endl;
}
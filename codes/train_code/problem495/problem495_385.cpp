...
signed main()
{
    ...
    for (int mask = 0; mask < (1 << n); mask++)
    {
        ...
        {
            for (int subsubmask = submask; subsubmask > 0; subsubmask = submask & (subsubmask - 1))
            {
                int x = 0, y = 0, z = 0;
                int ST = 10 * __builtin_popcount(mask);
                int m1 = mask - submask;
                int m2 = submask - subsubmask;
                int m3 = subsubmask;
                if (__builtin_popcount(m1) == 0) continue;
                if (__builtin_popcount(m2) == 0) continue;
                if (__builtin_popcount(m3) == 0) continue;
                forn(i, n) if ((1 << i) & m1) x += l[i];
                forn(i, n) if ((1 << i) & m2) y += l[i];
                forn(i, n) if ((1 << i) & m3) z += l[i];
                ans = min(ans, ST + abs(x - a) + abs(y - b) + abs(z - c));
            }
        }
    }
    cout << ans - 30;

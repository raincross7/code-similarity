    ...
    loop(bit1, 1 << N)
    {
        loop(bit2, 1 << N)
        {
            ...
            if (a.size() == 0 || b.size() == 0 || c.size() == 0)
                continue;
            ll MP = 0;
            ll preA = 0, preB = 0, preC = 0;
            MP += 10 * (a.size() + b.size() + c.size() - 3);
            loop(i, a.size()) preA += l[a[i]];
            loop(i, b.size()) preB += l[b[i]];
            loop(i, c.size()) preC += l[c[i]];
            MP += abs(A - preA) + abs(B - preB) + abs(C - preC);
            ans = min(ans, MP);
        }
    }
    putout(ans);

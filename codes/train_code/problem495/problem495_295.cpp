...
void solve(){
  ...
  V< int > l(n); cin >> l; int ans = IINF;
  sort( ALL(l) );
  FOR(i,0,1<<(2*n) ){
    int al = 0, bl = 0, cl = 0; int an=0, bn=0, cn=0;int cost = 0;
    FOR(j,0,n){
      int cc = (1&(i>>(2*j) ) ) + 2*(1&(i>>(2*j+1) ) );
      if(cc==1){
        al += l[j];
        an++;
      }
    ...
    if(an==0 || bn==0 || cn==0) continue;
    cost = 10*(an-1) + 10*(bn-1) + 10*(cn-1) + abs(a-al) + abs(b-bl) + abs(c-cl);
    ans = min(cost,ans);
  }
  coutn( ans );
}

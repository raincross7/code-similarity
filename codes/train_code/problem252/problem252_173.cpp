...
bool _compare(pair<int32_t, int64_t> a, pair<int32_t, int64_t> b)
{
	// 昇順
	if (a.second != b.second) {
		return a.second < b.second;
...
int main()
{
	...
	copy(va.begin(), va.end(), back_inserter(v));
	copy(vb.begin(), vb.end(), back_inserter(v));
	...
	//cout << accumulate(v.begin(), v.begin() + x + y, 0) << endl;
	//return 0;

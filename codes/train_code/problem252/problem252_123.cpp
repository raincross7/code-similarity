...
int main(){
  ...
  sort(begin_end(Bs));
  reverse(begin_end(Bs));
  sort(begin_end(Cs));
  reverse(begin_end(Cs));

  // まずは無色のりんご無しで、赤と緑のりんごだけを食べる
  // priority_queue<ll> 胃
  priority_queue<ll, vector<ll>, greater<ll> > stomach;
  ll ans = 0;
  rep(i, X){
    stomach.push(As[i]);
    ans += As[i];
  ...
  // 胃に入っているりんごを小さい順に出していって、無色のりんごのが美味しいのであれば、そのりんごをansから引き、無色のりんごを食べ直す
  ll c_idx = 0;
  while (!stomach.empty()){
    ll taste = stomach.top();
    stomach.pop();

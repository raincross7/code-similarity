...
int main(void)
{
	...
	vt<ll> p(A); each(i, e, p) cin >> e;
	vt<ll> q(B); each(i, e, q) cin >> e;
	vt<ll> c(C); each(i, e, c) cin >> e;

	sort(all(p), greater<ll>());
	sort(all(q), greater<ll>());
	vt<ll> ans;
	rep(i, 0, X) ans.push_back(p[i]);
	rep(i, 0, Y) ans.push_back(q[i]);

	sort(all(ans));
	sort(all(c), greater<ll>());
	rep(i, 0, Size(c))
	{
		if (Size(ans) <= i || c[i] < ans[i]) break;
		ans[i] = c[i];
	}

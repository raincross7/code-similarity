...
int main(void) {
    ll X, Y, A, B, C;
    cin >> X >> Y >> A >> B >> C;
    vector<ll> red(A), green(B), other(C);
    rep(i, A) cin >> red[i];
    rep(i, B) cin >> green[i];
    rep(i, C) cin >> other[i];
    sort(red.begin(), red.end(), greater<>());
    sort(green.begin(), green.end(), greater<>());
    rep(i, X) other.push_back(red[i]);
    rep(i, Y) other.push_back(green[i]);
    sort(other.begin(), other.end(), greater<>());
    ll delicious = 0;
    rep(i, X + Y) delicious += other[i];
    cout << delicious << endl;
    return 0;
}

...
public:
    ...
    vector<int> dijkstra(int start, vi &r) {
        vector<vector<int>> d(1 << R, vi(verN, INF));
        vector<int> mp(N, -1);
        REP(i, R)
        mp[r[i]] = i;
        auto newState = [&](int state, int n) {
            if (mp[n] != -1) {
                return state | (1 << mp[n]);
            }
            return state;
        };
        int state = newState(0, start);
        d[state][start] = 0;
        priority_queue<pair<int, pair<int, int>>, vector<pair<int, pair<int, int>>>, greater<pair<int, pair<int, int>>>> pq;
        vector<vector<int>> used(1 << R, vi(verN, 0));
        pq.push(make_pair(0, pi(start, state)));
        while (!pq.empty()) {
            ...
            for (auto adj_pair : adj_list[now]) {
                ...
                int nxts = newState(s, adj);
                if (used[nxts][adj])
                    continue;
                if (d[nxts][adj] > d[s][now] + dist) {
                    d[nxts][adj] = d[s][now] + dist;
                    pq.push(make_pair(d[nxts][adj], pi(adj, nxts)));
                }
            }
        }
        dump(d[(1 << R) - 1]);

...
int main(){
      ...
      train a[n-1];
      for(int i=0;i<n-1;i++){
            cin>>a[i].tp>>a[i].start>>a[i].freq;
      }
      for(int j=0;j<n-1;j++){
       time= a[j].tp+a[j].start;
      for(int i=j+1;i<n-1;i++){

           if(a[i].start>=time){
              time=a[i].start+a[i].tp;
           }
           else{
                  int num= time-a[i].start;
                  int next=(num+a[i].freq-1)/a[i].freq;
                   time= a[i].start + next*a[i].freq + a[i].tp;

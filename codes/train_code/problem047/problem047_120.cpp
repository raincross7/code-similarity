	...
	if (N >= 2) {
		rep(i, N - 1) {
			...
			for (int j = i; j < N - 1; j++) {
				...
				//待ち時間 (j での)
				int wait_time = 0;
				if(now_time % F.at(j) != 0 ) wait_time = max(F.at(j) - now_time % F.at(j), S.at(j) - now_time);
				else {
					if(S.at(j) - now_time > 0) wait_time = S.at(j) - now_time;
				}

				now_time += wait_time;
				//cout << i << ": " << j << " で待ち終わった: " << now_time << endl;

				//j + 1に行く。
				now_time += C.at(j);
				//cout << i << ": " << j+1 << " に着いた直後: " << now_time << endl;
			}

...
ll solve(ll ps, ll small, ll c) {
    if(c > k) return 0;
    if(ps == sz) return (c == k);
    if(dp[ps][small][c] != -1) return dp[ps][small][c];

    ll i, en = small ? 9 : (s[ps] - '0'), res(0);

    fora(i, 0, en) {
        res += solve(ps+1, small | (i < (s[ps]-'0')), c+(i != 0));
    }

    return dp[ps][small][c] = res;
}

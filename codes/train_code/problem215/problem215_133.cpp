...
int main(){
  ...
  if(k==1){
    ...
    lli ans=0;
    ans+=(str.length()-1)*(str.length()-2)/2*9*9
              + (int)(str[0]-'1')*(str.length()-1)*9;
    int num=1;
    for(num;num+1<str.length();num++)if(str[num]!='0')break;
            ans+= (str.length()-num-1)*9
              + (int)(str[num]-'0');
    cout << ans;
  }else if(k==3){
    lli ans=0;
    ans+=(str.length()-1)*(str.length()-2)*(str.length()-3)/6*9*9*9
              + (int)(str[0]-'1')*(str.length()-1)*(str.length()-2)/2*9*9;
    int num=1;
    for(num;num+1<str.length();num++)if(str[num]!='0')break;
            ans += (str.length()-num-1)*(str.length()-num-2)/2*9*9
              + ((int)(str[num]-'1')>0 ?(int)(str[num]-'1') :0)*(str.length()-num-1)*9;
              num=min(num+1,(int)str.length()-1);
    for(num;num+1<str.length();num++)if(str[num]!='0')break;
              ans += (str.length()-num-1)*9
              + (int)(str[num]-'0');
              cout <<ans;

...
void longestConnection(int cur, vector<vector<int>>&nodes, vector<bool>&visited, int &count){
    visited[cur] = true;
    count++;
    for(auto &node: nodes[cur]){
        if(!visited[node]){
            longestConnection(node, nodes, visited, count);
        }
    }
}
void solve(){
    ...
    fr(i,v){
        if(!visited[i]){
            longestConnection(i, nodes, visited, count);
            ans = max(ans, count);
            count = 0;
        }
    }

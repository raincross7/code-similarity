...
// Problem : B - Contiguous Repainting
// Contest : AtCoder - AtCoder Grand Contest 008
// URL : https://atcoder.jp/contests/agc008/tasks/agc008_b
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
int32_t main()
{
	...
	for(ll i=k;i<=n;i++){
		ans=max(ans, (p[i]-p[i-k]) + b[n]-(b[i]-b[i-k]));
		ans=max(ans,b[n]-(b[i]-b[i-k]));

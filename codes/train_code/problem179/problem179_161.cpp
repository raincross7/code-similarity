...
int main() {
    ...
    REP(i,n){
        ...
        sum_b[i+1] = sum_b[i]+max(0,a[i]);
    }

    ll ans = 0;
    REP(i,n-k+1){
//        cout << sum_a[i+k]-sum_a[i] << ":" << sum_b[i]-sum_b[0] << " " << sum_b[n]-sum_b[i+k] << endl;
        chmax(ans,max(0LL,sum_a[i+k]-sum_a[i])+(sum_b[i]-sum_b[0])+(sum_b[n]-sum_b[i+k]));
    }

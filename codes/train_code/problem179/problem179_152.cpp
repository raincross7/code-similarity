...
int main(void){
  int n, k, u; scanf("%d%d\n", &n, &k);
  Vint A(n), non_negative_A(n);
  Vlli subSum_A(n+1), subSum_non_negative_A(n+1);
  subSum_A[0] = 0; subSum_non_negative_A[0] = 0;
  REP(i, n){
    scanf("%d", &u); A[i] = u; non_negative_A[i] = ((u > 0) ? u : 0);
    subSum_A[i+1] = subSum_A[i] + u;
    subSum_non_negative_A[i+1] = subSum_non_negative_A[i] + non_negative_A[i];
  }
  lli res = -INFi;
  FOReq(i, 0, (const int)(n - k)){
    lli koho = subSum_non_negative_A[i] + subSum_non_negative_A[n] - subSum_non_negative_A[i + k],
        koho2 = subSum_A[i+k] - subSum_A[i];
    if(koho2 > 0) koho += koho2;
    if(res < koho) res = koho;
  }
  printf("%lld\n", res);
  return 0;
}
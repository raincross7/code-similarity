...
// あるK個の連続するマスは同じ色になる。
// 連続するマスの選び方は(N-K+1)通り，白黒のどちらで塗るかを考慮すると2*(N-K+1)通り。
// 「これらのマスのスコア+他のマスに書かれた整数の絶対値の総和」の最大値が答え。

int N, K;
vl a, Sa;
vl b, Sb;

ll solve(int i) { // [i,i+K)を同じ色で塗る
	ll res = (Sb[i] - Sb[0]) + (Sb[N] - Sb[i + K]); // [0,i),[i+K,N)を非負にしたときのスコア
	ll temp = Sa[i + K] - Sa[i];
	res += max(temp, 0LL);
	return res;
}
...
int main() {
	...
	rep(i, N) {
		...
		b[i] = max(a[i], 0LL); Sb[i + 1] = Sb[i] + b[i];
	}
	// dbl(a); dbl(b); dbl(Sa); dbl(Sb);
	ll ans = 0;
	rep(i, N - K + 1) chmax(ans, solve(i));
	cout << ans << endl;
}
...
signed main() {
  ...
  RSQsq myvec(n) // 宣言 nは必要な要素数
  myvec.add(i,k) // 配列のi番目に要素kを足す(更新ではないことに注意)
  myvec.at(i) // 配列のi番目にアクセス(0-index)
  myvec.rsum(i,j) // 配列の[i,j]の範囲の和を返す(0-index)

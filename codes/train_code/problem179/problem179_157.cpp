...
int main(){
    ll n,k;cin >> n >>k;
    SegTree<ll> al(n,0,[](ll a,ll b){return a+b;},[](ll a,ll b){return b;}),pl=al;
    rep(i,n){
        ll t;cin >> t;
        al.change(i,t);
        if(t>0)pl.change(i,t);
    }
    ll ans=0;
    rep(i,n-k+1){
        chmax(ans,pl.query(0,i)+pl.query(i+k,n));
        //cout << pl.query(0,i)+pl.query(i+k,n) <<endl;
    }
    rep(i,n-k+1){
        chmax(ans,pl.query(0,i)+pl.query(i+k,n)+al.query(i,i+k));
    }
    cout << ans <<endl;
}
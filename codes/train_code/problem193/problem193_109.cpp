...
char itocd(int i){char c = i + '0'; if(isdigit(c)) return c; else return ' ';}
char itocl(int i){char c = i + 'a'; if(islower(c)) return c; else return ' ';}
char itocu(int i){char c = i + 'A'; if(isupper(c)) return c; else return ' ';}
// ----------------------------------------------------------------
...
int main() {
    ...
    int N; cin >> N;
    int A[4]; rep(i, 4) {A[3-i] = N%10; N/=10;}
    string res = "";
    rep(i, 2<<3){
        bitset<3> bs = i;
        int calc = A[0];
        rep(i, 3){
            if(bs.test(i)) calc += A[i+1];
            else calc -= A[i+1];
        }
        if(calc == 7){
            res += itocd(A[0]);
            rep(i, 3){
                if(bs.test(i)) {res += '+'; res += itocd(A[i+1]);}
                else {res += '-'; res += itocd(A[i+1]);}
            }
            res += "=7";
            break;
        }

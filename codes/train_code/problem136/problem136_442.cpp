...
#define nvarep(n,a) int n;cin>>n;vector<ll>a(n);rep(i,0,n)cin>>a[i]
...
int main (){
    ...
    cin>>s>>t;
    sort(range(s));
    sort(range(t),greater<int>());
    rep(i,0,min(s.size(),t.size())){
        if(s[i]==t[i])continue;
        else if(s[i]<t[i]){cout<<"Yes"<<nnn;return 0;}
        else if(s[i]>t[i]){cout<<"No"<<nnn;return 0;}
    }
    if(s.size()<t.size())cout<<"Yes"<<nnn;
    else cout<<"No"<<nnn;
    return 0;
}
...
void pushdown(int u, int st, int en) {
    tr[u] += lz[u];
    if (st != en) {
        lz[u + u] += lz[u];
        lz[u + u + 1] += lz[u];
...
void update(int u, int st, int en, int l, int r, ll x) {
    ...
    if (r < st || en < l)
        ...
        int mid = (st + en) / 2;
        update(u + u, st, mid, l, r, x);
        update(u + u + 1, mid + 1, en, l, r, x);
...
ll query(int u, int st, int en, int l, int r) {
    ...
    if (r < st || en < l)
        ...
        int mid = (st + en) / 2;
        return (query(u + u, st, mid, l, r)+query(u + u + 1, mid + 1, en, l, r));
    }
}
...
ll n, d, a;
int q_right_point(int point, ll val) {
    int lo = point, hi = n - 1;
    ll ans = point, mid;
    while (lo <= hi) {
        mid = (lo + hi) / 2;
...
int main() {
    ...
    ll ans = 0;
    for (int i = 0; i < n; i++) {
        ll updated = query(1, 1, n, i+1, i+1);
        if (updated >= ara[i].ss)
            continue;
        int leftpoint = i;
        int rgpoint = q_right_point(i, ara[i].ff + d + d);
        ll cnt = (ara[i].ss - updated) / a + ((ara[i].ss - updated) % a ? 1 : 0);
        ll need = a * cnt;
        ans+=cnt;
        update(1, 1, n, leftpoint+1, rgpoint+1, need);
    }
    cout<<ans<<"\n";
}

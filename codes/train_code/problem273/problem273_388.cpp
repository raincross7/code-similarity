...
int main() {
  ...
  while (mi < N) {
    ...
    ll max_x = monster[mi].X + D * 2;
    Monster max_m = {max_x, 0};
    int max_mi = upper_bound(monster, monster + N, max_m) - monster;
    ll cnt = (h / A + (h % A > 0));
    DBG(cnt, tree.Get(mi), -cnt * A);
    tree.Update(mi, max_mi, -cnt * A);
    DBG(cnt, tree.Get(mi));
    // break;
    ans += cnt;
  }

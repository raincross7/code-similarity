...
signed main() {
  ...
  struct Monster {
    i64 x, h;
    Monster(i64 x = 0, i64 h = 0) :x(x), h(h) {}
  };
  ...
  auto c1 = [](const STE& l, const Monster& r) -> STE {
    STE o;
    return o;
  };
  auto c2 = [](STE& l, const STL& r, int elementCount) -> void {
    l.h = l.h > r.Sub ? l.h - r.Sub : 0;
  };
  auto c3 = [](STL& l, const STL& r) -> void {
    l.Sub += r.Sub;
  };
  auto qSub = [](i64 value) -> STL {
    STL l;
    l.Sub = value;
    return l;
  };

  SegmentTree<STE, STL> st(N, c1, c2, c3);
  ...
  auto cm = [](const Monster& l, const Monster& r) {
    return l.x < r.x;
  };
  ...
  F0R(i, N) {
    ...
    if (h) {
      ...
      i64 rx = (i64)m.x + (i64)D * 2;
      int r = lower_bound(rng(ms), Monster(rx + 1), cm) - ms.begin();

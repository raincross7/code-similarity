...
int main(void) {
    ...
    REP(i, n) {
        int y, h;
        cin >> y >> h;
        x[i] = make_pair(y, (h + a - 1) / a);
    }
    sort(ALL(x));
    vpprint(x);
    VLL r(n);
    REP(i, n) {
        auto it = upper_bound(ALL(x), make_pair(x[i].first + 2 * d, (LL)INF));
        it--;
        r[i] = it - x.begin();
    }
    vprint(r);
    VLL cnt(n+1, 0);
    cnt[0] = x[0].second;
    cnt[r[0] + 1] = -x[0].second;
    LL ans = cnt[0];
    FOR(i, 1, n) {
        cnt[i] += cnt[i-1];
        if(cnt[i] < x[i].second) {
            cnt[r[i] + 1] -= x[i].second - cnt[i];
            ans += x[i].second - cnt[i];
            cnt[i] += x[i].second - cnt[i];
        }
    }
    // vprint(cnt);
    cout << ans << endl;

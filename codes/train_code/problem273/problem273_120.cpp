...
bool cmp(ll a, pair<ll, ll> const &b)
{
    return a < b.first;
}
...
void Main()
{
    ...
    ll ret = 0;
    auto leftItr = M.begin();
    rep(leftN, N)
    {
        leftItr++;

        M[leftN].second -= P[leftN];
        ...
        debugvar(leftN);
        debugvartab(M[leftN], 1);
        debugvartab(P[leftN], 1);

        int time = divup(M[leftN].second, A);
        ...
        P[leftN] += time * A;
        int rightN = upper_bound(leftItr, M.end(), M[leftN].first + D * 2, cmp) - M.begin();
        P[rightN] -= time * A;

        debugvartab(rightN, 1);

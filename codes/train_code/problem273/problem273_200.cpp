...
bool isOK(int index, LL key, VPLL &p){
  return p.at(index).FI <= key;
}

int BinarySearch(int i, int d, VPLL &p){
  int ok = i;
  int ng = SZ(p);
  while((ng-ok)>1){
    int mid = (ok+ng)/2;
    if(isOK(mid,p.at(i).FI+d*2,p)) ok = mid;
    else ng = mid;
  }
  return ng;
}
...
int main() {
  ...
  VPLL p(n);
  REP(i,n) cin >> p.at(i).FI >> p.at(i).SE;
  ...
  REP(i,n){
    if(s.at(i)<p.at(i).SE){
      LL cnt = (p.at(i).SE -s.at(i) +a-1)/a;
      int t = BinarySearch(i,d,p);
      ans += cnt;
      s.at(i) += cnt*a;
      s.at(t) -= cnt*a;
    }

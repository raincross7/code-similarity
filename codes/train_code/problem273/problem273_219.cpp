...
void solve(int n, int d, int a, vp xh) {
  SORT(xh);
  vi ins(n + 1, INF), end(n, n), cum(n + 1);
  rep(i, n)ins[i] = xh[i].first;
  for(int l = 0, r = 1; l < n && r < n + 1;) {
    if(ins[r] - ins[l] <= d * 2)r++;
    else {
      end[l] = r;
      l++;
    }
  ...
  int ans = 0;
  rep(i, n) {
    int cnt = max(0ll, ceil_div(xh[i].second, a) - cum[i]);
    ans += cnt;
    cum[i] += cnt;
    cum[end[i]] -= cnt;
    cum[i + 1] += cum[i];

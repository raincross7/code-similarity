...
void solve(){
  ...
  int mx=-1,idx=-1,mx1=-1;
  rep(i,0,n){
    if(mx<v1[i].f) mx=v1[i].f,idx=i;
    else if(mx==v1[i].f && v1[idx].s>v1[i].s) idx=i;
  }
  mx1=v1[idx].s;
  vector<int> sol;
  rep(i,0,n) if(i!=idx && v1[i].s>mx) sol.pb(v1[i].s);
  sort(all(sol),greater<int>());
  int j=0,cnt=0;
  while(h>0){
    if(j<sol.size()) h-=sol[j++];
    else {
      int rem=(h+mx-1)/mx,rem1 = (h-mx1+mx-1)/mx + 1;
      cnt+= min(rem,rem1);
      break;
    }
    cnt++;

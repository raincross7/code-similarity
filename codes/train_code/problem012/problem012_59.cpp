...
int main(){
  ...
  // <攻撃力, 種類:= 1:何回も使える 0:1回しか使えない
  priority_queue<pair<ll, ll> > weapons;
  rep(i, N){
    ll a = il();
    ll b = il();
    weapons.push(m_p(a, 1));
    weapons.push(m_p(b, 0));
  }

  ll ans = 0;
  while (H > 0){
    pair<ll, ll> nextweapon = weapons.top();
    if (nextweapon.second == 1){
      ans += ceil_divide(H, nextweapon.first);
      H = 0;
    }
    else{
      ans++;

...
template<typename Type> inline void sort(Type& arr) { std::sort(arr.begin(), arr.end(), std::less<>()); }
template<typename Type> inline void rsort(Type& arr) { std::sort(arr.begin(), arr.end(), std::greater<>()); }

...
void solve()
{
    ll c, d, n, m, l, r, k, x, y, z, ans = 0ll;
    string s, t;
    cin>>n;
    VLL a(n), b(n);
    REP(i,n) cin>>a[i];
    ...
    ll diff_a = 0ll, diff_b = 0ll;
    REP(i,n) {
        sum_a += a[i];
        sum_b += b[i];
        if (a[i] > b[i]) diff_b += a[i] - b[i];
        else if (a[i] < b[i]) diff_a += (b[i] - a[i] + 1) / 2;
    }
    Yes(diff_a <= sum_b - sum_a && diff_b <= sum_b - sum_a);
}

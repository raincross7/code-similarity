...
void solve(long long N, long long K){
    ll ans = 0;
    ll sum = mulMod(N,N+1);
    sum = mulMod(sum,powMod(2,MOD-2));
    for (ll i = K; i <= N+1; i++)
    {
        ll sub = mulMod(N-i,N-i+1);
        sub = mulMod(sub,powMod(2,MOD-2));
        ll maximum = sum - sub;
        if(maximum < 0)maximum+=MOD;

        ll minimum = mulMod(i-1,i);
        minimum = mulMod(minimum,powMod(2,MOD-2));

        ll tmp = maximum - minimum;
        if(tmp < 0)tmp+=MOD;
        tmp++;
        tmp %= MOD;
        ans += tmp;

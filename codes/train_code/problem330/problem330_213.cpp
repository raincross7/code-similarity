...
void solve(int N, int M, vi a, vi b, vi c) {
  ...
  vvb used(N, vb(N));
  rep(s, N - 1)rep(g, s + 1, N) {
    function<void(vi,int)> dfs = [&](vi v, int n) {
      int sb = wf[s][v.back()];
      each(next, adj[v.back()]) {
        if(wf[s][next.first] != sb + next.second)continue;
        vi nv(all(v));
        nv.push_back(next.first);
        if(next.first == g) {
          rep(i, n){
            used[nv[i]][nv[i + 1]] = true;
            used[nv[i + 1]][nv[i]] = true;
          }
        } else {
          dfs(nv, n + 1);
        }

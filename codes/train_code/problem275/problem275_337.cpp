...
int main() {
  ...
  rep(i,n){
    if(a[i]==1){
      maxs(state0.first,x[i]);
    }
    if(a[i]==2){
      mins(state.first,x[i]);
    }
    if(a[i]==3){
      maxs(state0.second,y[i]);
    }
    if(a[i]==4){
      mins(state.second,y[i]);
    }
    if(state.first-state0.first<=0||state.second-state0.second<=0){
    cout<<0<<endl;
    return 0;
    }
  }
  int ans=(state.first-state0.first)*(state.second-state0.second);
  cout<<ans<<endl;
}

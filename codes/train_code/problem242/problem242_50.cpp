...
int main() {
	...
	ll odd = (abs(v[0].first)+ abs(v[0].second)) % 2;
	REPC(p, v) {
		if ((abs(p.first) + abs(p.second)) % 2 != odd) {
			cout << -1 << endl;
			return 0;
		}
	}
	...
	VL tNum;
	const ll num = 32 + (odd + 1) % 2;
	tNum.reserve(num);
	REP(i, (odd + 1) % 2) { tNum.emplace_back(1); }
	ll t = 1;
	REP(i, 32) {
		tNum.emplace_back(t);
		t *= 2;
	}
	std::reverse(ALL(tNum));
	...
	REPC(p, v) {
		...
		ll v = x - y;
		REPC(t, tNum) {
			if (u >= 0 && v >= 0) { cout << "R"; u -= t; v -= t; } else if (u < 0 && v >= 0) { cout << "D"; u += t; v -= t; } else if (u >= 0 && v < 0) { cout << "U"; u -= t; v += t; } else/* if (u < 0 && v < 0)*/ { cout << "L"; u += t; v += t; }
		}
		cout << endl;
		// cout << (u + v) / 2 << " ";
		// cout << (u - v) / 2 << endl;

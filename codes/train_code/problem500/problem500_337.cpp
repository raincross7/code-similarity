...
signed main() {
	...
	rep(i, N) {
		if (s.at(i) != 'x') {
			N2++;
			//s_except_x.push_back(s.at(i));
			//pos.push_back(i);
		}
	}

	vector<char> s_except_x(N2);
	vector<int> pos(N2);

	int j = 0;
	rep(i, N) {
		if (s.at(i) != 'x') {
			s_except_x.at(j) = s.at(i);
			pos.at(j) = i;
			j++;
		}
	}

	//int N2 = s_except_x.size();
	int x_number = N - N2;

	bool able = true;
	rep(i, N2 / 2) {
		if (s_except_x.at(i) != s_except_x.at(N2 - 1 - i)) {
			able = false;
			break;
		}
	}
	...
	if (!able) {
		...
		if (N2 % 2 == 1) {
			rep(i, N2 / 2) {
				res += abs((pos.at(N2 / 2 + 1 + i) - pos.at(N2 / 2 + i)) - (pos.at(N2 / 2 - i) - pos.at(N2 / 2 - 1 - i)));
			}
		}
		else {
			if (N2 > 2) {
				rep(i, N2 / 2 - 1) {
					res += abs(pos.at(N2 / 2 + 1 + i) - pos.at(N2 / 2 + i) - (pos.at(N2 / 2 - 1 - i) - pos.at(N2 / 2 - 2 - i)));
				}
			}
			
		}

		res += abs(N - 1 - pos.at(N2 - 1) - pos.at(0));

...
signed main(){
	...
	rep(0,i,m)rep(0,j,n+1)dp[i][j]="";
	sort(a,a+m);
	rep(1,i,n+1){
		if(i%a[0].s==0){
			rep(0,j,i/a[0].s)dp[0][i]+=a[0].f;
		}	
		else dp[0][i]="0";
	}		
	rep(1,i,m){
		rep(1,j,n+1){
			...
			string k="";
			if(dp[i][j-a[i].s]=="0"&&j!=a[i].s)k="";
			else k=a[i].f+max(dp[i][j-a[i].s],dp[i-1][j-a[i].s]);
			dp[i][j]=maxst(k,dp[i-1][j]);
		}	
	}	
	c(dp[m-1][n])
}	
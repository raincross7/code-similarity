...
int main() {
	...
	vector<SINT64> data(9);
	vector<SINT64> ct = {2,5,5,4,5,6,3,7,6};
	...
	rep(i,0,N) {
		...
		rep(j,0,9) {
			if (data[j] == 1) {
				...
				SINT64 a = j+1;
				buf = buf+to_string(a);
				if (dp[i+ct[j]].length() < buf.length()) {
					dp[i+ct[j]] = buf;
//					puts(i);puts(i+ct[j]);put(buf);
				} else if (dp[i+ct[j]].length() == buf.length()) {
					if (dp[i+ct[j]][buf.length()-1] < buf[buf.length()-1]) {
						dp[i+ct[j]] = buf;
//						puts(i);puts(i+ct[j]);put(buf);
					}
				}

...
int main() {
    ...
    REP(i, N + 1) {
        ...
        for (int k:A) {
            if (i + C[k - 1] > N)continue;
            if (dp[i + C[k - 1]].size() == (dp[i] + to_string(k)).size())
                dp[i + C[k - 1]] = max(dp[i + C[k - 1]], dp[i] + to_string(k));
            else if (dp[i + C[k - 1]].size() < (dp[i] + to_string(k)).size())
                dp[i + C[k - 1]] = dp[i] + to_string(k);
        }
    }
    PRI(dp[N])
    return 0;

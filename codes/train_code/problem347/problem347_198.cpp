...
int main(){
    myset();
    vector<int> need = {inf,2,5,5,4,5,6,3,7,6};
    int N,M;
    cin >> N >> M;
    vector<int> can(M);
    rep(i,M)cin >> can[i];
    ...
    rep(i,2,N + 1){
        for(int x : can){
            if(i < need[x])continue;
            if(memo[i - need[x]] == ""){
                if(i - need[x] == 0)chmax(memo[i],to_string(x));
            }
            else{
                chmax(memo[i],memo[i - need[x]] + (char)(x + '0'));
                chmax(memo[i],(char)(x + '0') + memo[i - need[x]]);
            }
        }
    }
    cout << memo[N] << "\n";

...
int main(){
	...
	//R
	if(p.sd + 1 < w){
	    if( s[p.ft][p.sd] == '.' && s[p.ft][p.sd + 1] == '#')t = 1;
	    else t = 0;
	    
	    if(d[p.ft][p.sd + 1] > d[p.ft][p.sd] + t){
		d[p.ft][p.sd + 1] = d[p.ft][p.sd] + t;
		q.push({p.ft,p.sd+ 1});
	    }	   
	}
	//D
       	if(p.ft + 1 < h){
	    if( s[p.ft][p.sd] == '.' && s[p.ft + 1][p.sd] == '#')t  = 1;
	    else t = 0;

	    if(d[p.ft  +1][p.sd] >  d[p.ft][p.sd] + t){
		d[p.ft + 1][p.sd] = d[p.ft][p.sd] + t;
		q.push({p.ft + 1,p.sd});
	    }
	}
    }

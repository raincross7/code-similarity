...
// dfs(各頂点を既に見たかどうか, その頂点「より前」にいくつ頂点をとったか, 今回とる頂点)
ll dfs(vector<ll> picked, ll pickcnt, ll t){
  picked[t] = 1;
  if (++pickcnt==N) return 1;
  ll res = 0;
  for (ll next : adj_list[t]){
    if (picked[next] == 0){
      res += dfs(picked, pickcnt, next);
    }
    // 既にとった頂点に対しては何もしない
  }
  return res;  
...
int main(){
  ...
  adj_list = vector<vector<ll> >(N, vector<ll>() ); 
  ...
  vector<ll> picked(N, 0);
  cout << dfs(picked, 0, 0) << endl;

...
vi adj[MAX_N],vis(MAX_N),curr_t(MAX_N),nodes;
map<int,int>in_tm,out_tm,idx;
int cntr = 1;
...
void solve(){
    ...
    // for(auto x:in_tm) cout << x.first << " " << x.second << '\n';
    // cout << '\n';
    // for(auto x:out_tm) cout << x.first << " " << x.second << '\n';
    // for(auto x:nodes) cout << x << " - " << idx[x] <<'\n';

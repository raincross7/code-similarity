...
void aaa(ll target, ll total_counter, Graph& tree, ll parent) {
  ll point = counter[target] + total_counter;
  ans[target] = point;
  REP(i, tree[target].size()) {
    if (tree[target][i] != parent) {
      aaa(tree[target][i], point, tree, target);
    }
  }
}

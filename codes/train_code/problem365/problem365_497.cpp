...
#endif
  // let (x3, y3) = (0, 0)
  // the area(s/2) will be |(x2y1 - x1y2) / 2 |
  // just find x2y1 - x1y2 = s
  // limit is 0 ~ 1e9
  // set x2 = 1e9, y2 = 1
  // so find 1e9y - x = s
  // y = (s + x) / 1e9
  // s + x should be 1e9's multiple
  // x can be (1e9 - s % 1e9) % 1e9
  ll s;
  cin >> s;
  int x1, y1, x2 = 1e9, y2 = 1, x3 = 0, y3 = 0;
  x1 = (ll)(1e9 - s % (ll)1e9) % (ll)1e9;
  y1 = (s + x1) / 1e9;
  printf("%d %d %d %d %d %d\n", x1, y1, x2, y2, x3, y3);
  return 0;
}

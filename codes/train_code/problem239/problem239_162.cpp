...
void solve(){
    ...
    rep(i,k.size()+1){
        // cout << S.substr(0,i) << " " << k.substr(0,i) << " " << S.substr(n+i-k.size(),k.size()-i) << " " << k.substr(i, k.size()-i) << endl;
        if((S.substr(0,i) == k.substr(0,i)) && (S.substr(n+i-k.size(),k.size()-i) == k.substr(i, k.size()-i))){
            ok = true;
        }
    }

...
int main(){
    ...
    sort(p,p+m);
    ll c=1;
    vec<pair<ll,pair<ll,ll>>> v(m);
    rep(i,m){
        v[i].first=p[i].second;
        if(i>=1 && p[i].first.first==p[i-1].first.first) c++;
        else c=1;
        v[i].second.first=abs(p[i].first.first);
        v[i].second.second=c;
    }
    sort(ALL(v));
    rep(i,m){
        cout<<setw(6)<<setfill('0')<<v[i].second.first;
        cout<<setw(6)<<setfill('0')<<v[i].second.second<<endl;
    }
    return 0;
}

...
signed main() {
  ...
  for(int i=0;i<m;i++){
    ...
    cout << setw(6) << setfill('0') << input[i].first+1;
    auto iter = find(pref[input[i].first].begin(),
                     pref[input[i].first].end(),
                    input[i].second);
    cout << setw(6) << setfill('0') << iter-pref[input[i].first].begin()+1 << endl;
  }

...
int main(){
    ...
    vector<vector<P>> list(n+2);
    rep(i,m){
        cin>>p[i]>>y[i];
        list[p[i]].push_back(P(y[i],i));
    }
    rep(i,n+1){
        sort(ALL(list[i]));
    }
    rep(i,n+1){
        rep(j,list[i].size()){
            string ken=to_string(i);
            int zero=6-(int)ken.length();
            string old=to_string(j+1);
            int zero1=6-(int)old.length();
            NUM[list[i][j].se]=string(zero,'0')+ken+string(zero1,'0')+old;
        }
    }
    rep(i,m){
        cout<<NUM[i]<<endl;

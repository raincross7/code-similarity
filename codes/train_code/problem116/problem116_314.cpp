...
int main() {
  ...
  rep(i,m){
    vals.at(p.at(i)).push_back(y.at(i));
  }
  ...
  rep(v,n){
    ...
    vals.at(v).erase(unique(vals.at(v).begin(),vals.at(v).end()), vals.at(v).end());
  }
  ...
  rep(i,m){
    ...
    //座標圧縮して導いた答え
    int id = lower_bound(vals.at(v).begin(),vals.at(v).end(),y.at(i)) - vals.at(v).begin();
    printf("%06d\n", id + 1);
    }
}

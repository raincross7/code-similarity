...
ll Npow(ll x, ll n) {
	if (n == 0) return 1;
	if (n % 2 == 0) return Npow(x * x % MOD, n / 2);
	else return x * Npow(x, n - 1) % MOD;
}

int main() {
	ll D;
	cin >> N >> M >> D;
	if (D == 0) cout << fixed << setprecision(10) << (ld)(M - 1) / N;
	else cout << fixed << setprecision(10) << (ld) (M - 1) * 2 * max(z,(N - D)) / N / N;
}
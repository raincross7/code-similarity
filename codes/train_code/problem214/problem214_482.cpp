...
void DP_stairs() { lint N, M; std::cin >> N >> M;
	std::vector<bool> issafe(N + 1, true); std::vector<lint> dp(N + 1, 0);
	for (int i = 0; i < M; ++i) { int a; cin >> a; issafe[a] = false; }
	dp[0] = 1; if (issafe[1] == true) dp[1] = 1;
	for (int i = 2; i <= N; ++i) { if (issafe[i - 1]) dp[i] += dp[i - 1]; 
	if (issafe[i - 2]) dp[i] += dp[i - 2]; dp[i] %= NUM97;}
	std::cout << dp[N] << std::endl;}

int main() {
	lint n, K;  std::cin >> n >> K;
	std::vector<lint> dp; dp.resize(100000);  for (auto& r : dp) r = INF;
	std::vector<lint> h; h.resize(1000000); for (auto& r : h) std::cin >> r;
	dp[0] = 0; 
	for (int i = 0; i < n; ++i) {
		for (int j = 1; j <= K; ++j) {

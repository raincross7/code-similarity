...
int main() {
    ...
    for(int i = 0;i<N;i++)for(int j = 0;j<N;j++) cin >> ban.at(i).at(j);
    for(int i = 0;i<M;i++)for(int j = 0;j<M;j++) cin >> temp.at(i).at(j);
        ...
        for(int j = 0;j<N;j++){
            ...
            for(int k = 0;k<M;k++){
                for(int l = 0;l<M;l++){
                    ...
                    if(ban.at(k+i).at(l+j) != temp.at(k).at(l)) ok = false;
                }
            }
            if(ok){
                cout << "Yes" << endl;

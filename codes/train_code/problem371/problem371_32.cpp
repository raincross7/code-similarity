...
signed main() {
  ...
  string sSub = s.substr(0,(s.length()-1)/2);
  string sSubR = sSub;
  reverse(sSubR.begin(), sSubR.end());
  if(sSub!=sSubR){
    cout << "No" << endl;
    return 0;
  }
  
  string sSub2 = s.substr((s.length()+3)/2 - 1, s.length() - ( (s.length()+3)/2-1) );
  string sSubR2 = sSub2;
  reverse(sSubR2.begin(), sSubR2.end());
  if(sSub2!=sSubR2){
    cout << "No" << endl;
    return 0;
  }

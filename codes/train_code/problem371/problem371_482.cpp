...
int main(){
	...
	std::reverse(t.begin(),t.end());
	std::reverse(t.begin(),t.begin()+t.size()/2);
	if(s.substr(0,s.size()/2)!=t.substr(0,s.size()/2)){
		puts("No");
		return 0;
	}
	std::reverse(t.begin(),t.begin()+t.size()/2);
	std::reverse(t.begin()+t.size()/2+1,t.end());
	if(s.substr(s.size()/2+1,s.size()/2)!=t.substr(s.size()/2+1,s.size()/2)){
		puts("No");
	}
	else puts("Yes");
	return 0;

...
int main() {
	...
	for(int i=0; i<N-1; i++) {
		...
		a--; b--;
		node[a].eg.pushb(b);
		node[a].egcost.pushb(c);
		node[b].eg.pushb(a);
		node[b].egcost.pushb(c);
	}
	...
	while(!s.empty()) {
		int now=s.top(); s.pop();
		for(int i=0; i<node[now].eg.size(); i++) {
			int next=node[now].eg[i];
			int ncost=node[now].egcost[i];
			if(node[next].reached) continue;
			node[next].cost=node[now].cost+ncost;
			node[next].reached=true;
			s.push(next);
		}
	}
	...
	for(int i=0; i<Q; i++) {
		int x,y; cin>>x>>y;
		x--; y--;
		cout<<node[x].cost+node[y].cost<<endl;
	}
}

...
int main()
{
  ...
  vector<ll> distance(N,1e18); distance[K] = 0;
  
  while(!q.empty()){
    ll now = q.front(); q.pop();
    isSearched[now] = true;
    map<ll,ll> mp = graph[now];
    for(auto itr = mp.begin(); itr != mp.end(); itr ++){
      ll next = itr->first, c = itr->second;
      if(!isSearched[next] && distance[next] > distance[now] + c){

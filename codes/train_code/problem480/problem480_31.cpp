...
signed main() {
  ...
  vector<ll> distFromK(n);
  vector<bool> flag(n,false);
  queue<pair<int,ll>> q; q.emplace(k,0);
  while(!q.empty()){
    auto node = q.front(); q.pop();
    flag[node.first] = true;
    distFromK[node.first] = node.second;
    for(auto ed:graph[node.first]){
      if(flag[ed.first]==true)continue;
      q.emplace(ed.first, node.second+ed.second);
    }
  }

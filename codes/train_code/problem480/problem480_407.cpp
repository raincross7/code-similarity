...
int main() {
  ...
  for (int i = 1; i < N; i++) {
    ...
    E.to = B - 1;
    E.cost = C;
    W.at(A - 1).push_back(E);
    E.to = A - 1;
    W.at(B - 1).push_back(E);
  }
  int Q, K;
  cin >> Q >> K;
  vector<ll> A(N, LINF);
  ...
  while (!PQ.empty()) {
    ...
    for (int i = 0; i < int(W.at(S).size()); i++) {
      edge E = W.at(S).at(i);
      if (A.at(E.to) > A.at(S) + E.cost) {
        A.at(E.to) = A.at(S) + E.cost;
        LP MP(A.at(E.to), E.to);
        PQ.push(MP);
      }
    }
  }

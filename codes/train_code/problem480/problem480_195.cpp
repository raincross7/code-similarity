...
signed main(){
  ...
  rep(i,n-1){
    ...
    aa--;bb--;
    g[aa].push_back(P(bb, cc));
    g[bb].push_back(P(aa, cc));
  }
  cin>>q>>k;k--;
  vector<int> dis(n, -1);
  stack<P> st;st.push(P(k, 0));
  while(!st.empty()){
    P p = st.top();st.pop();
    dis[p.first] = p.second;
    rep(i,g[p.first].size()){
      if(dis[g[p.first][i].first] == -1)st.push(P(g[p.first][i].first, p.second + g[p.first][i].second));
    }
  }
  rep(i,q){
    int x, y;cin>>x>>y;

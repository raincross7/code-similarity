...
using Graph=vector<vector<Edge> >;//ll or Edge
using Edge=pair<int,ll>;
// g++ -o a a.cpp -Wall -lm -std=c++14
//tree探索dfsを復習siro
void rec(const Graph &G,int v,int p,ll sum,VI &dist){
    dist[v]=sum;
    for(auto u:G[v]){
        if(u.fi==p)continue;
        rec(G,u.fi,v,sum+u.se,dist);
    }
}
...
int main(){
    ...
    int q,k;cin>>q>>k;k--;
    VI dist(n,0);
    rec(G,k,-1,0,dist);//-1は最初どっち方向に行っても良いということ.みたい
    rep(i,q){
        int x,y;cin>>x>>y;x--;y--;

...
void solve(int N, vi a, vi b, vi c, int Q, int K, vi x, vi y) {
  vvp adj(N);
  rep(i,N-1){
  ...
  function<void(int)> dfs = [&](int i) {
    each(e, adj[i]) {
      if(cost[e.first] > -1)continue;
      cost[e.first] = cost[i] + e.second;
      dfs(e.first);
    }
  };
  dfs(K);
  rep(i, Q) COUT(cost[x[i]] + cost[y[i]]);
}
...
signed main() {
  ...
  solve(N, move(a), move(b), move(c), Q, K, move(x), move(y));
}

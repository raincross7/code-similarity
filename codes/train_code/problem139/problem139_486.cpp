...
signed main(){
	...
	rep(i,1<<N){
		rep(j,N){
			if(i>>j&1)continue;
			int index=i|(1<<j);
			if(MAX1[index].first<MAX1[i].first){
				MAX2[index]=MAX1[index];
				MAX1[index]=MAX1[i];
			}else if(MAX2[index].first<MAX1[i].first&&MAX1[index].second!=MAX1[i].second){
				MAX2[index]=MAX1[i];
			}
			if(MAX2[index].first<MAX2[i].first){
				MAX2[index]=MAX2[i];
			}
		}
	}
	REP(i,1<<N){
		ans[i]=MAX1[i].first+MAX2[i].first;

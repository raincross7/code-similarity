...
signed main() {
	...
	rep(i, 1 << n) {
		rep(j, n) {
			if (i >> j & 1)continue;
			int k = i | (1 << j);
			if (f[k] <= f[i] && f[k].second != f[i].second) {
				s[k] = f[k];
				f[k] = f[i];
				if (s[k] < s[i] && s[k].second != s[i].second)s[k] = s[i];
			}
			else if (s[k] < f[i] && s[k].second != f[i].second && f[k].second != f[i].second) {
				s[k] = f[i];
			}
		}
	}

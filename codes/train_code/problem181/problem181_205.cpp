...
int main(){
  ...
  if (a + 2 >= b || k <= a){
    // aよりbのが2以上大きくないと、交換するインセンティブが無い
    cout << k + 1 << endl;
    return (0); 
  }
  // ↑ex) 5ターンあって、a=5なら、4ターンためてやっと5ターン目に持金5になっても、1円にかえて終わり= 意味がない。よって、k <= a ならやっぱり叩くだけ
  // ここからは、まず持ちビスケットがaになるまでため、その後2ターンあれば交換、1ターンしかなければ叩く
  ll ans = 1; 
  k -= (a - 1); // aたまるまで叩く
  ans += a-1;

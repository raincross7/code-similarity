...
void prep(vi &to,string s,char ck){
    int ct=0;
    F(i,0,s.size()){
        if(s[i]==ck){
            ct++;
            if(i==s.size()-1)to.pb(ct);
        }
        else if(ct && s[i]!=ck){
            to.pb(ct);
            ct=0;
...
int main(){
    ...
    prep(Z,s,'0');
    prep(O,s,'1');
    int ans=0,zln=0,oln=0,lseg=0;

    for(int i=0;i<Z.size()||i<O.size();i++){
        if(i<O.size())oln += O[i];
        if(i<Z.size())zln += Z[i];
        ...
        int x=0;
        if(s[0]=='0' && lseg>0)x += O[lseg-1];
        if(s[0]=='1' && i+1<O.size())x += O[i+1];

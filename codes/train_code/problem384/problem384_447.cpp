...
void solve(long long N, long long K, std::string S){
    ...
    for (int i = 0; i < N-1; i++)
    {
        if(S[i+1] != S[i]){
            cnt++;
            compress.push_back(make_pair(cnt,S[i]-'0'));
            cnt=0;
        }else{
            cnt++;
        }
    ...
    cnt++;
    compress.push_back(make_pair(cnt,S[N-1]-'0'));
    N = compress.size();
    ll ans = -1;
    ll right = 0;
    ll sum = 0;
    ...
    {
        while(right < N && cnt <= K){
            if(cnt == K && compress[right].second == 0) break;
            sum += compress[right].first;
            if(compress[right].second == 0){
                cnt++;
            }
            ++right;
        }
        ...
        if(right == left) ++right;
            ...
            sum -= compress[left].first;
            if(compress[left].second == 0){
                cnt--;
            }
        }
    }

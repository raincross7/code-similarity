...
int main()
{
  ...
  // freopen("input.txt","r",stdin);
    ...
    while(cin>>n){
        ...
        if(a[1]%2) dp[1][1] = a[1]/2;

        for(int i=2; i<=n; i++){

            dp[i][0] = dp[i-1][0] + a[i]/2;
            if(a[i]%2 && dp[i-1][1]) dp[i][0] = max(dp[i][0], dp[i-1][1] + a[i]/2 + 1);


            if(a[i]==0) continue;

            if(a[i]%2) dp[i][1] = max(dp[i-1][0],dp[i-1][1]) + a[i]/2;
            else{
                if(dp[i-1][1]) dp[i][1] = dp[i-1][1] + (a[i]-1)/2 + 1;
            }

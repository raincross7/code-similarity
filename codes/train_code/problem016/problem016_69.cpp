#include<bits/stdc++.h>
#define rep(i,n) for(int i=0;i<(n);++i)
#define FOR(i,n,j) for(int i=(j);i<(n);++i)
using namespace std;
using ll=long long;
using ld=long double;
typedef pair<int,int> P;

class Calc_Mod{//calculate mod
    public:
        unsigned int mod=1000000007;
        ll plus_mod(ll a,ll b){
            return (a+b)%mod;
        }
        ll minus_mod(ll a,ll b){
            return (a-b)%mod;
        }
        ll multi_mod(ll a,ll b){
            return a*b%mod;
        }
        ll LSM(ll a,int b){//a^b%mod
            queue<int> q;
            while(b>=1){
                q.push(b%2);
                b/=2;
            }
            ll ans=1;
            while(!q.empty()){
                if(q.front()==1){
                    ans=multi_mod(ans,a);
                }
                a=multi_mod(a,a);
                q.pop();
            }
            return ans;
        }
        ll div_mod(ll a,ll b){// a/b
            return multi_mod(a,LSM(b,mod-2));
        }
        ll combi(ll a,ll b){//aCb
            ll ans=1;
            ll kaijo=1;
            rep(i,b){
                ans=multi_mod(ans,a-i);
                kaijo=multi_mod(kaijo,b-i);
            }
            ans=div_mod(ans,kaijo);
            return ans;
        }
};

int main()
{
    Calc_Mod mod;
    
    int N;
    cin >> N;
    vector<ll> a;
    vector<int> count_bit;
    count_bit.resize(60);
    a.resize(N);
    rep(i,N)cin>> a[i];
    vector<queue<int>> bin;
    bin.resize(N);
    rep(i,60)count_bit[i]=0;
    ll val;

    ll two_=1;
    rep(i,N){
        val=a[i];
        while(val>=1){
            bin[i].push(val%2);
            val/=2;
        }
    }
    int j;
    rep(i,N){
        j=0;
        while(!bin[i].empty()){
            count_bit[j++]+=bin[i].front();
            bin[i].pop();
        }
    }
    ll ans=0;
    rep(i,60){
        ans=mod.plus_mod(ans,mod.multi_mod(two_,mod.multi_mod(N-count_bit[i],count_bit[i])));
        two_=mod.multi_mod(two_,2);
    }
    cout << ans << endl;
}
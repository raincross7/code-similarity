...
int main(){
    ...
    for(int i=0; i<N; i++){
        ...
        Table[x.first].push_back(x.second);
        min_day = min(x.first, min_day);
    }
    llong total = 0;
    llong now_day = min_day;

    while(now_day<=M){
        for(int j=0; j<Table[now_day].size(); j++){
            PQ.push(Table[now_day][j]);
        }
        if(PQ.empty()){
            now_day++;
            continue;

...
signed main()
{
    ...
    vector<pii> ans;
    if(m & 1){
        for(int i = 1; i <= m/2; i++) ans.pb({i, m - i + 1});
        for(int i = 1; sz(ans) != m; i++) ans.pb({m + i, 2 * m + 2 - i});
    }
    else{
        
        for(int i = 1; i <= m/2; i++) ans.pb({i, m - i + 2});
        for(int i = 1; sz(ans) != m; i++) ans.pb({m + i + 1, 2 * m + 2 - i});
    }

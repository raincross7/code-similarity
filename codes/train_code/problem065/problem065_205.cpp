...
queue<string> target_q;
vs results;

void aaa(string target) {
  char end = target[target.size() - 1];
  if (end == '0') {
    target_q.push(target + '0');
    target_q.push(target + '1');
  }
  else if (end == '9') {
    target_q.push(target + '8');
    target_q.push(target + '9');
  }
  else {
    ll int_end = end - '0';
    target_q.push(target + to_string(int_end - 1));
    target_q.push(target + end);
    target_q.push(target + to_string(int_end + 1));
  }
}
...
void Main() {
  ...
  REP(i, 40000) {
    string target = target_q.front();
    aaa(target);
    results.pb(target_q.front());
    target_q.pop();
  }

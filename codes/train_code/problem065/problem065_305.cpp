...
int main(){
  ...
  ll k = 9; // これまでに見つかったルンルン数の個数
  
  // 前の桁のルンルン
  set<string> pre_lunlun = {"1", "2", "3", "4", "5", "6", "7", "8", "9"};


  string nowlun = "";
  while (k < K){
    vector<string> new_lunlun;
    for (string prelun : pre_lunlun){
      bool found = false;
      REP(diff, -1, 2){
        // 変化量 -1, 0, 1
        int to_append = int(*prelun.rbegin() - '0') + diff;
        if (0 <= to_append && to_append <= 9){
          nowlun = prelun + to_string(to_append);
          new_lunlun.push_back(nowlun);
          if (++k == K){
            found = true;
            break;
          }
    ...
    // 全てのprelunについて新しいlunlunを作った
    pre_lunlun.clear();
    rep(i, new_lunlun.size()) pre_lunlun.insert(new_lunlun[i]);
  }

...
int main(int argc, const char * argv[])
{
    ...
    vector<string>kazu[100][10];
    for(int i=0;i<10;i++){
        stringstream ss;
        ss<<i;
        kazu[0][i].push_back(ss.str());
    }
    if(K<10){
        cout << K << endl;
    }
    ...
    while(cnt<=K){
        for(int i=0;i<10;i++){
            ...
            ss<<i;
            if(i!=0){
                for(int j=0;j<kazu[keta-1][i-1].size();j++){
                    kazu[keta][i].push_back(ss.str()+kazu[keta-1][i-1][j]);
                    cnt++;
                    if(cnt==K){
                        cout << ss.str()+kazu[keta-1][i-1][j] << endl;
                    }
                }
            }
            for(int j=0;j<kazu[keta-1][i].size();j++){
                kazu[keta][i].push_back(ss.str()+kazu[keta-1][i][j]);
                if(i!=0){
                    cnt++;
                }
                if(cnt==K){
                    cout << ss.str()+kazu[keta-1][i][j] << endl;
                }
            }
            if(i!=9){
                for(int j=0;j<kazu[keta-1][i+1].size();j++){

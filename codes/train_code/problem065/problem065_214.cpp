...
int main() {
  ...
  int K;
  cin >> K;
  vector<set<string>> L(10);
  for (int i = 1; i <= 9; i++) {
    L.at(0).insert(to_string(i));
  }
  for (int i = 1; i < 10; i++) {
    for (auto value : L.at(i - 1)) {
      if (value.at(value.size() - 1) == '0') {
        L.at(i).insert(value + '1');
      }
      else if (value.at(value.size() - 1) == '9') {
        L.at(i).insert(value + '8');
      }
      else {
        L.at(i).insert(value + to_char(ctoi(value.at(value.size() - 1)) - 1));
        L.at(i).insert(value + to_char(ctoi(value.at(value.size() - 1)) + 1));
      }
      L.at(i).insert(value + value.at(value.size() - 1));
    }
  }
  int ans = 0;
  for (int i = 0; i < 10; i++) {
    if (ans + L.at(i).size() >= K) {
      for (auto value : L.at(i)) {
        ans++;
        if (ans == K) {
          cout << value << endl;
        }

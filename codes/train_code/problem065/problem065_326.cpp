...
string getNextLunLun(string numStr){
  int l = numStr.length();
  int nextV = 0;
  int carryOverDigit = -1;
  for(int i=0;i<l; i++){
    int d = numStr[i]-'0';
    if(d==9){
      //carry over
    ...
    if(i==l-1) continue;
    int nextD = numStr[i+1]-'0';
    if(d==nextD+1){
      //carry over
      carryOverDigit = i;
    }
    else{
      //Ready to find next LunLun
      break;
    }
  }

  if(carryOverDigit==l-1) {
    for(int i=0;i<l; i++){
      numStr[i]='0';
    }
    numStr += '1';
  ...
  char baseNum = numStr[carryOverDigit+1];
  for(int i=0;i<=carryOverDigit;i++){
    numStr[carryOverDigit-i] = baseNum;
    if(baseNum>'0') baseNum--;
  }
  numStr[carryOverDigit+1]++;
  return numStr;
}

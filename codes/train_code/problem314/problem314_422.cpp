...
signed main() {
    ...
    // DPのように見える
    // 既に払ってしまった金額をキーとして、そこから10万に行くために必要な回数

    vi Cnt(MAX_PRICE+1, -1);
    Cnt.at(N) = 0;
    ...
    REPD(i, N) {
        ...
        int prev_p = i + 1;
        if (Cnt.at(prev_p) >= 0) {
          int cnt = Cnt.at(prev_p) + 1;
          ans = min(ans, cnt);
        }

        FORE(six_cnt, 1, 6)  {
            int prev_p = i + pow(6, six_cnt);
            if (prev_p <= MAX_PRICE && Cnt.at(prev_p) >= 0) {
                int cnt = Cnt.at(prev_p) + 1;
                ans = min(ans, cnt);
            }
        }

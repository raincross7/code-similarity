...
ll fac[FACSIZE];
ll n;
char getS(ll i, ll j, vector<vector<char>> &S)
{
  ...
  REP(i, n)
  {
    REP(j, n)
    {
      ...
      if (j == 0)
      {
        ...
        for (ll k = 0; k < n; k++)
        {
          for (ll l = k + 1; l < n; l++)
          {
            if (k == l)
              continue;
            //char val1 = S[k + pos.first][l + pos.second];
            char val1 = getS(k + pos.first, l + pos.second, S);
            char val2 = getS(l + pos.first, k + pos.second, S);
            OUT2(val1, val2);
            if (val1 == val2)
              cntInit++;
          }
      ...
      else
      {
        ...
        REP(k, n - 1)
        {
          char val1 = getS(pos.first + n - 1, pos.second + k, S);
          char val2 = getS(pos.first + k, pos.second + n - 1, S);
          OUT2(val1, val2);
          if (val1 == val2)
            cntIn++;
        }
        REP(k, n - 1)
        {
          char val1 = getS(pos.first - 1, pos.second + k, S);
          char val2 = getS(pos.first + k, pos.second - 1, S);
          OUT2(val1, val2);
          if (val1 == val2)
            cntOut++;
        }
        dpS[i][j] = dpS[i][j - 1] + cntIn - cntOut;
        OUT2(cntIn, cntOut);
      }

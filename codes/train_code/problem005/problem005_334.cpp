...
void solve() {
  ...
  vector<RollingHash> srh, trh;
  rep(i, n * 2) {
    srh.push_back(RollingHash(S[i % n]));
    trh.push_back(RollingHash(T[i % n]));
  }
  int ans = 0;
  rep(i, n) {
    bool ok = true;
    rep(k, n - 1) {
      if (srh[i + k].get(k + 1, n) != trh[k].get(i + k + 1, i + n)) {
        ok = false;
        break;
      }
    }

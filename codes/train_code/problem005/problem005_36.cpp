...
int fun(int x,int y) {
    vector<int> v;
    int ans = 0;
    for(int d = 0; x + d <= 2 * n - 1 && y + d <= 2 * n - 1; d++) {
        int i1 = x + d, j1 = y + d;
        int i2 = x + d, j2 = y + d;
        int c = 0;
        while(i1 <= 2 * n - 1 && j1 <= 2 * n - 1 && i2 <= 2 * n - 1 && j2 <= 2 * n - 1) {
            if(a[i1][j1] != a[i2][j2]) {
                break;
            }
            else {
    ...
    for(int i = sz(v) - 1; i >= 0; i--) {
        while(!d.empty() && d.back().fi >= v[i]) d.pop_back();
        d.push_back({v[i], i});
        while(!d.empty() && d.front().se <= i - n) d.pop_front();
        if(sz(v) - i >= n) {
            if(d.front().fi - i >= n) {
                ans++;
            }

...
int main() {
	...
	rep(i, h) {
		string s;
		cin >> s;
		rep(j, w)table[i][j] = s[j] == '.';
	}
	vvint yoko(h, vint(w)), tate(h, vint(w));
	rep(i, h) {
		rep(j, w) {
			if (table[i][j]) {
				if (j == 0)yoko[i][j]++;
				else yoko[i][j] = yoko[i][j - 1] + 1;
			}
		}
		drep(j, w - 1) {
			if (yoko[i][j] != 0)yoko[i][j] = max(yoko[i][j], yoko[i][j + 1]);
		}
	}
	rep(j, w) {
		rep(i, h) {
			if (table[i][j]) {
				if (i == 0)tate[i][j]++;
				else tate[i][j] = tate[i - 1][j] + 1;
			}
		}
		drep(i, h - 1) {
			if (tate[i][j] != 0)tate[i][j] = max(tate[i][j], tate[i + 1][j]);
		}
	}
	int ans = 0;
	rep(i, h) {

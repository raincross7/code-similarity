...
int main()
{
	...
	static int countLR[2001][2001];
	static int countRL[2001][2001];
	static int countUD[2001][2001];
	static int countDU[2001][2001];
	repi(n)
	{
		reads(s[i], m + 1);
		zero(countLR[i], m);
	...
	repi(n)
	{
		if (s[i][0] == '.') countLR[i][0] = 1;
		repj1(m)
		{
			if (s[i][j] == '.') countLR[i][j] = countLR[i][j - 1] + 1;
		}
	}
	repi(n)
	{
		if (s[i][m-1] == '.') countRL[i][m-1] = 1;
		for (int j = m - 2; j >= 0; --j)
		{
			if (s[i][j] == '.') countRL[i][j] = countRL[i][j+1] + 1;
		}
	}

	repi(m)
	{
		if (s[0][i] == '.') countUD[0][i] = 1;
		repj1(n)
		{
			if (s[j][i] == '.') countUD[j][i] = countUD[j-1][i] + 1;
		}
	}
	repi(m)
	{
		if (s[n - 1][i] == '.') countDU[n - 1][i] = 1;
		for (int j = n - 2; j >= 0; --j)
		{
			if (s[j][i] == '.') countDU[j][i] = countDU[j+1][i] + 1;
		}
	}
	...
	repi(n)
	{
		repj(m)
		{
			int a = countLR[i][j] + countRL[i][j] - 2;
			int b = countUD[i][j] + countDU[i][j] - 2;
			int k = a + b + 1;
			if (k > best) best = k;
		}
	}

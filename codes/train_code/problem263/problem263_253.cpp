...
// in(vec) works for size-defined type vectors
	...
	while (t--) {
		...
		vector<vll>temp(h, vll(w, 0));
		vector<vll>suff(h, vll(w, 0));
		vector<vll>up(h, vll(w, 0));
		vector<vll>down(h, vll(w, 0));
		...
		forn(i, 0, h) {
			forn(j, 0, w) {
				if (temp[i][j] && up[i][j] && down[i][j] && suff[i][j]) {
					maxa = temp[i][j] + up[i][j] + down[i][j] + suff[i][j] - 3;
				}
				ans = max(maxa, ans);

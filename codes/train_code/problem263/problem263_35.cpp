...
int main(){
    ...
    char s[h][w];
    V<V<int>> ro(h),co(w);
    rep(i,h)ro[i].push_back(-1);
    rep(i,w)co[i].push_back(-1);
    rep(i,h){
        rep(j,w){
            cin>>s[i][j];
            if(s[i][j]=='#'){
    ...
    rep(i,h)ro[i].push_back(w);
    rep(i,w)co[i].push_back(h);
    int ans=0;
    rep(i,h){
        rep(j,w){
            if(s[i][j]=='.'){
                int x=lb(all(ro[i]),j)-ro[i].begin();
                int y=lb(all(co[j]),i)-co[j].begin();
                int cnt=ro[i][x]-ro[i][x-1]+co[j][y]-co[j][y-1]-3;
                chmax(ans,cnt);
            }
        }
    }

...
const int nn = 2005;
ll m1[nn][nn], m2[nn][nn], m3[nn][nn], m4[nn][nn];
void solve(){
    ll n, m; cin >> n >> m;
    char a[n+1][m+1];
    rep(i, n) rep(j, m) cin >> a[i+1][j+1];
    for(ll i = 1; i <= n; i++)
        for(ll j = 1; j <= m; j++) m1[i][j] = (a[i][j] == '.' ?  m1[i-1][j] + 1 : 0 );
    for(ll i = n; i >= 1; i--)
        for(ll j = 1; j <= m; j++) m2[i][j] = (a[i][j] == '.' ?  m2[i+1][j] + 1 : 0 );
    for(ll j = 1; j <= m; j++)
        for(ll i = 1; i <= n; i++) m3[i][j] = (a[i][j] == '.' ?  m3[i][j-1] + 1 : 0 );
    for(ll j = m; j >= 1; j--)
       for(ll i = 1; i <= n; i++)  m4[i][j] = (a[i][j] == '.' ?  m4[i][j+1] + 1 : 0 );

    //for(ll i = 1; i <= n; i++) {for(ll j = 1; j <= m; j++) cout << m2[i][j] << " " ;  cout << endl; }

    ll ans = 0;
    for(ll i = 1; i <= n; i++)
        for(ll j = 1; j <= m; j++) 
            if(a[i][j] == '.')ans = max(ans, m1[i-1][j] + m2[i+1][j] + m3[i][j-1] + m4[i][j+1] + 1);

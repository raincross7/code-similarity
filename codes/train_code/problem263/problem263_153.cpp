...
int main() {
	...
	vector<vector<P>> xr(h, vector<P>(w));
	rep(i, h) {
		int left = 0;
		int right = 0;
		rep(j, w) {
	...
	vector<vector<P>> yr(h, vector<P>(w));
	rep(j, w) {
		int top = 0;
		int bottom = 0;
		rep(i, h) {
	...
	rep(i, h) {
		rep(j, w) {
			if (grid[i][j]) {
				ll x = xr[i][j].second - xr[i][j].first;
				ll y = yr[i][j].second - yr[i][j].first;
				chmax(u, x+y-1);
			}
		}
	}

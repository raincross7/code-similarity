...
//    readint(test_cases);
    ...
    forr(t, 1, test_cases) {
        ...
        vector<vector<int>> dist_to_left(h+2, vector<int>(w+2));
        forr(i,0,h+2) {
            int curr = 0;
            forr(j,0,w+2) {
                if (grid[i][j]=='#') {  curr = 0;  }
                else {  curr++; dist_to_left[i][j] = curr-1;   }
            }
        }
        vector<vector<int>> dist_to_right(h+2, vector<int>(w+2));
        forr(i,0,h+2) {
            int curr = 0;
            for(int j=w+1; j >= 0; --j) {
                if (grid[i][j]=='#') {  curr = 0;  }
                else {  curr++; dist_to_right[i][j] = curr-1;   }
            }
        }
        vector<vector<int>> dist_to_up(h+2, vector<int>(w+2));
        forr(j,0,w+2) {
            int curr = 0;
            forr(i,0,h+2) {
                if (grid[i][j]=='#') {  curr = 0;  }
                else {  curr++; dist_to_up[i][j] = curr-1;   }
            }
        }
        vector<vector<int>> dist_to_down(h+2, vector<int>(w+2));
        forr(j,0,w+2) {
            int curr = 0;
            for(int i=h+1; i >= 0; --i) {
                if (grid[i][j]=='#') {  curr = 0;  }
                else {  curr++; dist_to_down[i][j] = curr-1;   }
            }
        }
        ...
        forr(i,0,h+2) {
            forr(j,0,w+2) {
                if (grid[i][j]=='.') {
                    int right = dist_to_left[i][j], left = dist_to_right[i][j];
                    int up = dist_to_up[i][j], down = dist_to_down[i][j];

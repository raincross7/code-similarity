...
int main() {
  ...
  VVI l(h,VI(w,0)), r(h,VI(w,0));
  VVI u(h,VI(w,0)), d(h,VI(w,0));

  REP(i,h){
    l.at(i).at(0) = s.at(i).at(0)=='.';
    r.at(i).at(w-1) = s.at(i).at(w-1)=='.';
  }
  REP(i,w){
    u.at(0).at(i) = s.at(0).at(i) == '.';
    d.at(h-1).at(i) = s.at(h-1).at(i) == '.';
  }
  REP(i,h){
    FOR(j,1,w){
      if(s.at(i).at(j)=='#') l.at(i).at(j)=0;
      else l.at(i).at(j) = l.at(i).at(j-1)+1;
    }
    REPR(j,w-1){
      if(s.at(i).at(j)=='#') r.at(i).at(j)=0;
      else r.at(i).at(j) = r.at(i).at(j+1)+1;
    }
  }

  REP(j,w){
    FOR(i,1,h){
      if(s.at(i).at(j)=='#') u.at(i).at(j)=0;
      else u.at(i).at(j) = u.at(i-1).at(j)+1;
    }
    REPR(i,h-1){
      if(s.at(i).at(j)=='#') d.at(i).at(j)=0;
      else d.at(i).at(j) = d.at(i+1).at(j)+1;
    }
  }
  ...
  REP(i,h)REP(j,w){
    ...
    chmax(ans,l.at(i).at(j)+r.at(i).at(j)+u.at(i).at(j)+d.at(i).at(j)-3);
  }

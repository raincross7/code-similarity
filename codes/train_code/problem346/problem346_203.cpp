...
int main() {
  ...
  for(auto c:mpx){
    if(c.second==ansx){
      vx.push_back(c.first);
    }
  }
  for(auto c:mpy){
    if(c.second==ansy){
      vy.push_back(c.first);
    }
  }
  ll cnt=vx.size()*vy.size();
  rep(i,m){
    if(binary_search(ALL(vx),p[i].first) && binary_search(ALL(vy),p[i].second)) cnt--;
  }
  if(cnt>=1) cout<<ansx+ansy<<endl;
  else cout<<ansx+ansy-1<<endl;
}
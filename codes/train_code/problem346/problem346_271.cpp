...
int main()
{
  ...
  //main
  ll hm = 0, wm = 0; set<ll> hs, ws;
  FOR(i,H){
    hm = max(hm,hn[i]);
  }
  FOR(i,H){
    if(hn[i] == hm){
      hs.emplace_hint(hs.end(),i);
    } 
  }
  FOR(j,W){
    wm = max(wm,wn[j]);
  ...
  FOR(j,W){
    if(wn[j] == wm){
      ws.emplace_hint(ws.end(),j);
    } 
  }
  
  ll ans = hm+wm-1;
  for(auto hitr = hs.begin(); hitr != hs.end(); hitr ++){
    for(auto witr = ws.begin(); witr != ws.end(); witr ++){
      ll a = *hitr, b = *witr;
      if(target[a][b]){
        continue; 
      }else{

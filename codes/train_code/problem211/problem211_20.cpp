...
int main(){
    ...
    x[k] = make_pair(2,2);
    for(int i=k-1;i>0;i--){
        const auto& mi = x[i+1].first;
        const auto& ma = x[i+1].second;

        const ll low = mi;
        x[i].first = (mi+a[i]-1)/a[i-1];
        x[i].first = (low+a[i-1]-1)/a[i-1]*a[i-1];

        ll up = (ma/a[i]+1)*a[i]-1;
        x[i].second = up/a[i-1]*a[i-1];
    }

    x[0].first = x[1].first;
    x[0].second = (x[1].second/a[0]+1)*a[0]-1;

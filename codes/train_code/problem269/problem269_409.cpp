...
ll x, y, depth;  // キューから取り出した座標
ll cnt;
ll ma;
vector<vector<char>> vec(1000, vector<char>(1000));//int型の2次元配列(3×4要素の)の宣言queue<Corr> q;
vector<vector<char>> moto(1000, vector<char>(1000));//int型の2次元配列(3×4要素の)の宣言queue<Corr> q;
vector<vector<ll>> suu(1000, vector<ll>(1000));//int型の2次元配列(3×4要素の)の宣言queue<Corr> q;
queue<Corr> q;

vector<ll> x_vec = {0, 0, -1, +1};  // 進む方向の簡便化，テクニックとして覚えておく．
vector<ll> y_vec = {-1, +1, 0, 0};  // ループで回すことで上下左右に移動できるようになっている

ll bfs() {
    while (!q.empty()) {
        Corr now = q.front(); q.pop();  // キューの先頭を取り出す
        x = now.x, y = now.y, depth = now.depth;  // 行，列が y, x になっているので注意
        ...
        for (ll i = 0; i < 4; ++i) {
            Corr next = {x + x_vec[i], y + y_vec[i], depth + 1};
            if (0 <= next.y && next.y <= w-1 && 0 <= next.x && next.x <= h-1 && vec[next.x][next.y] == '.') {
                vec[next.x][next.y] = '#';  // 同じ場所を探索しないようにする
                suu[next.x][next.y] = next.depth;
//              	ma = max(ma,next.depth);
                q.push(next);
            }
        }
    }

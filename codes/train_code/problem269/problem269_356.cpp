...
int main() {
    ...
    while (!q.empty()) {
        ll i = q.front().first;
        ll j = q.front().second;
        if (i + 1 < h && !b[i + 1][j]) { q.push(make_pair(i + 1, j)); b[i + 1][j] = true; ans[i + 1][j] = ans[i][j] + 1; }
        if (j + 1 < w && !b[i][j + 1]) {
            q.push(make_pair(i, j + 1)); b[i][j + 1] = true; ans[i][j + 1] = ans[i][j] + 1;
        }
        if (i != 0 && !b[i - 1][j]){
        q.push(make_pair(i - 1, j)); b[i - 1][j] = true; ans[i - 1][j] = ans[i][j] + 1;
    }
        if (j != 0 && !b[i][j - 1]) {
            q.push(make_pair(i, j - 1)); b[i][j - 1] = true; ans[i][j - 1] = ans[i][j] + 1;
        }

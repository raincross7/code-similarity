...
bool update()
{
    ...
    // }
    bool f = true;
    rep(i, h) rep(j, w) if (t[j][i] == '.') f = false;
    if (f)
        return false;
    char _t[1020][1020];
    rep(i, h) rep(j, w)
...
signed main()
{
    ...
    queue<ppi> q;
    rep(1020) rep(j, 1020) visited[i][j] = false;
    rep(j, h)
    {
        string s;
        cin >> s;
    ...
    rep(i, h)
    {
        rep(j, w) max_d = max(max_d, dist[i][j]);
    }
    cout << max_d << endl;
    return 0;
}
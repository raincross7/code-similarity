...
signed main() {
  ...
  auto power = [&](int from, int to) {
    return abs(p[from].first - p[to].first) + abs(p[from].second - p[to].second);
  };

  vector<int> dist(h*w+1);
  rep(i, h*w+1-d) dist[i+d] = dist[i] + power(i,i+d);

  int q; cin >> q;
  while(q--) {
    int l,r; cin >> l >> r;
    cout << dist[r] - (l>=d ? dist[l] : 0) << endl;
  }
}

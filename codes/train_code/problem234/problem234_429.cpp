...
signed main(){
	...
	int sa[d][h*w/d+1];
	ic(q)
	rep(0,i,q)cin>>l[i]>>r[i];
	rep(0,i,d){
		int sum=0;
		rep(0,j,(h*w+d-i)/d){
			sa[i][j]=sum;
			sum+=abs(memh[i+j*d]-memh[i+(j+1)*d])+abs(memw[i+j*d]-memw[i+(j+1)*d]);
		}
	}	
	rep(0,i,q)c(sa[(r[i]-1)%d][(r[i]-1)/d]-sa[(l[i]-1)%d][(l[i]-1)/d])
}		

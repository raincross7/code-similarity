...
void solve() {
    REP(i, q) {
        ...
        int rind = (r - first) / d;
        // print(mod, lind, rind);
        int ans = cumdist[mod][rind] - cumdist[mod][lind];
        print(ans);
    }
}
signed main() {
    cin >> h >> w >> d;
    coord.resize(h * w + 1);
    dist.resize(d, vector<int>((h * w + d - 1) / d));
    cumdist.resize(d, vector<int>((h * w + d - 1) / d + 1));
    ...
    REP(di, d) {
        int cnt = 0;
        for (int diff = di + 1; diff < h * w + 1 - d; diff += d) {
            int dis = abs(coord[diff].first - coord[diff + d].first) +
                      abs(coord[diff].second - coord[diff + d].second);
            dist[di][cnt] = dis;
            cnt++;
        }
    }
    REP(di, d) {
        for (int i = 0; i < (h * w + d - 1) / d; ++i) {
            cumdist[di][i + 1] = cumdist[di][i] + dist[di][i];
        }
    }

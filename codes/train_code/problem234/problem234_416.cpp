...
int main(void) {
    ...
    V memo_field(H*W+1);
    //メモ初期化
    for(int i = H*W ; i < H*W-D ; i--)  memo_field[i] = 0;
    
    for(int i = 0 ; i < D ; i++){
      for(int j = H*W-D-i; 1 <= j ; j -= D){
        memo_field[j] = memo_field[j+D] + abs(A[j+D].first - A[j].first) + abs(A[j+D].second - A[j].second);
      }
    }
    ...
    REP(i, Q){
      int L, R;  cin >> L >> R;
      cout << memo_field[L] - memo_field[R] << endl;
    }

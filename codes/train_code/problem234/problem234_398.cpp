...
int main(){
    ...
    vector<ll> dist(N * M);
    for(ll i = D;i < N * M;i++){
        //dist[i + 1] = abs(ma[i].first - ma[i + 1].first) + abs(ma[i].second - ma[i].second);
        dist[i] = abs(ma[i].first - ma[i - D].first) + abs(ma[i].second - ma[i - D].second);
    }
    //vecout(dist);
    rep(i,D){
        //dist[i + 1] += dist[i];
        for(ll j = i + D;j < N * M;j += D){
            dist[j] += dist[j - D];
        }
    }
    //vecout(dist);
    rep(i,Q){
        cout << dist[to[i]] - dist[from[i]] << "\n";
    }
}
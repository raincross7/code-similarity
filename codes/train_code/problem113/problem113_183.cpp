...
int main() {
	...
	//rep(i, m) {
	//	inv[i] = mod_pow((ll)i, MOD - 2ll, MOD);
	//}
	//inv[0] = 1;
	for (int k = 1; k <= n + 1; k++) {
		//P(k);
		ll moto = fact[n + 1] * inv(fact[k]) % MOD*inv(fact[n + 1 - k]) % MOD;
		if (l + n - r < k - 1) {
			P(moto);
		}
		else {
			ll kaburi = fact[l + n - r] * inv(fact[k-1]) % MOD*inv(fact[l + n - r-(k-1)]) % MOD;
			P( (moto+MOD - kaburi)%MOD);
		}
	}
	return 0;
}

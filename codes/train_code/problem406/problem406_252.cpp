...
int main(){
  ...
  rep(i,n){
    rep(j,60){
      if(DEC(a[i],2,j)==1&&DEC(c,2,j)==1)a[i]-=POW2(2,j);
    }
  }
  per(i,60){
    c=0;
    rep(j,n){
      if(c==0&&DEC(a[j],2,i)==1&&a[j]/POW2(2,i)==1){
        c=a[j];
        t=j;
      }
      else if(DEC(a[j],2,i)==1&&c!=0)a[j]^=c;
    }
    rep(j,n){
      if(DEC(a[j],2,i)==1&&c!=0&&t!=j)a[j]^=c;
    }
  }
  c=0;
  rep(i,n)c^=a[i];

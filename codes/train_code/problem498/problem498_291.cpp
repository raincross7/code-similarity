...
void solve(void)
{
  ...
  auto hunowa = std::accumulate(minus.begin(), minus.end(), 0LL);
  if (hunowa == 0) {
    cout << 0 << CRLF;
    return;
  }
  hunowa = -hunowa;
  std::sort(plus.begin(), plus.end(), std::greater<ll>());

  vector<ll> P_SUM(plus.size());
  boost::partial_sum(plus, P_SUM.begin());
  for (int i = 0; i < plus.size(); ++i) {
    if (P_SUM[i] >= hunowa) {
      cout << (i+1 + minus.size()) << CRLF;
      return;
    }
  }
  cout << -1 << CRLF;

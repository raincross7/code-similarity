...
void solve(){
	...
	vvl dp(n+1,vl(2,1));
	auto f=[&](auto f, int u, int pr)->void{
		for(auto i:v[u])if(i!=pr)f(f,i,u);
		for(auto i:v[u])if(i!=pr)dp[u][1]=PROD(dp[u][1],SUM(dp[i][0],dp[i][1]));
		for(auto i:v[u])if(i!=pr)dp[u][0]=PROD(dp[u][0],dp[i][1]);
	};
	f(f,1,1);

	cout<<SUM(dp[1][0],dp[1][1])<<ENDL;
}

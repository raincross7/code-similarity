...
#define print2D(x) 									for(auto i : (x)){for(auto j : i){cout<<j<<" ";}nl;}nl;
#define pb 													push_back
#define mp 													make_pair
...
//vr color;
ll n;
ll dfs(ll src,ll par,vr &color)
{
	if(dp[src][color[par]]!=-1)
	{
		return dp[src][color[par]];
	}
	if(color[par]==1)
	{
		ll ans1=1,ans2=1;
		for(auto i : g[src])
		{
			...
			{
				color[src]=1;
				ans1=modmul(ans1,dfs(i,src,color),M1);
			}
		}
		for(auto i : g[src])
		{
			...
			{
				color[src]=0;
				ans2=modmul(ans2,dfs(i,src,color),M1);
			}
		}
		color[src]=-1;
		return dp[src][color[par]]= modadd(ans1,ans2,M1);
	}
	else if(color[par]==0)
	{
		ll ans=1;
		for(auto i : g[src])
		{
			...
			{
				color[src]=1;
				ans=modmul(ans,dfs(i,src,color),M1);
			}
		}
		color[src]=-1;
		return dp[src][color[par]]=ans;
	}
}
void solve(){}
void ask(){}

...
vi adj[100005];
ll dp[100005][2];
// dp[i][0] - no. of ways of coloring subtree of i when i is white
// dp[i][1] - no. of ways of coloring subtree of i when i is black
...
void dfs(int u, int p=-1){
    ...
    for(int v : adj[u]){
        if(v==p) continue;
        cws=(cws*dp[v][0])%MOD;
        cbs=(cbs*(dp[v][1]+dp[v][0]))%MOD;
    }
    dout << u << ": cws="<<cws << ", cbs="<<cbs<<endl;
    dp[u][0] = cbs;
    dp[u][1] = cws;
}

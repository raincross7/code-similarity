...
int main () {
  ...
  rep(i,n)
  {
    pii best = make_pair(0,-10000);//idx, 値
    rep(j,n)
    {
      if(R[j].first < B[i].first && R[j].second < B[i].second && Rused[j] == false)
      {
        if(R[j].second > best.second)
        {
          best.first = j;
          best.second = R[j].second;
        }
      }
    }
    
    if(best.second != -10000)
    {
      Rused[best.first] = true;
      ans++;
    }
  }
  cout << ans << endl;

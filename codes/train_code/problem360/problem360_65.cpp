...
void solve() {
    ...
    rep(i,N){
        int a,b;cin>>a>>b;
        Blue.push_back(P(a,b));
    }
    int res=0;
    bool ch[N]; fill(ch,ch+N,false);

    auto xsort = [](P left, P right){return left.first==right.first ? left.second < right.second : left.first < right.first;};
    auto ysort = [](P left, P right){return left.second==right.second ? left.first < right.first : left.second < right.second;};
    sort(all(Red), xsort);
    sort(all(Blue), xsort);

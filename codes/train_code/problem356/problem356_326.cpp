...
int main(){
    ...
    rep(i,0,N) mp[A[i]]++;
    ll val[300010] = {};
    SegmentTree sg(N+1, val);

    for(auto &e: mp){
        if(e.second >= 2){
            sg.add_val(1, e.second, 1);
            sg.add_val(e.second, e.second+1, -(e.second-1));
        }
        val[e.second] += e.second;
        val[N+1] -= e.second;
    }
    rep(i,0,N+2) val[i+1] += val[i];
    rep(i,1,N+1) val[i] += sg.query_sum(0, i+1);

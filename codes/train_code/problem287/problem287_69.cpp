...
void solve(){
  int n; cin >> n;
  V< int > v(n); cin >> v;
  int ans = 0; V2< pair<int,int> > vs(2, V<pair<int,int>>(100'050));
  FOR(k,0,2){
    FOR(i,0,n/2) vs[k][v[2*i+k]].fs++;
    FOR(i,0,100'020) vs[k][i].sd = i;
    sort(RALL(vs[k]));
  }
  //FOR(k,0,2) {FOR(i,0,5) couts(vs[k][i] ); coutn("");}
  if(vs[0][0].sd!=vs[1][0].sd){ ans = n - vs[0][0].fs - vs[1][0].fs;}
  else{
    ans = n - max<int>(vs[0][0].fs+vs[1][1].fs, vs[0][1].fs+vs[1][0].fs);
  }
  coutn( ans );
}

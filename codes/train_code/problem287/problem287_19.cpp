...
void solve() {
    ...
    rep(i,N){
        if(i%2==1){map1[A[i]]++;}
        else if(i%2==0){map2[A[i]]++;}
    }

    ll mn1,mn2,sn1,sn2; mn1=mn2=sn1=sn2=-1;

    for(auto a: map1){
        if(mn1<0 || map1[mn1]<a.second)mn1=a.first;
    }
    for(auto a: map2){
        if(mn2<0 || map2[mn2]<a.second)mn2=a.first;
    }

    for(auto a: map1){
        if(a.first==mn1)continue;
        if(sn1<0 || map1[sn1]<a.second)sn1=a.first;
    }
    for(auto a: map2){
        if(a.first==mn2)continue;
        if(sn2<0 || map2[sn2]<a.second)sn2=a.first;
    }
    ...
    if(mn1!=mn2){
        ...
        if(sn1==-1 && sn2==-1){
            ...
            chmin(res,  (co-map1[mn1]) + (co-map2[sn2]));
            chmin(res,  (co-map1[sn1]) + (co-map2[mn2]));
        }
        else if(sn1==-1){
            res = co-map2[sn2];
        }

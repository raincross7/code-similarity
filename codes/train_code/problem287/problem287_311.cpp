...
bool greater_nCnt(const StNumCnt& lhs, const StNumCnt& rhs)
{
...
int fnMinChangeCnt(const vector<int>& cnrvnNum)
{
  ...
  sort(begin(voNumCntEvn), end(voNumCntEvn), greater_nCnt);
  sort(begin(voNumCntOdd), end(voNumCntOdd), greater_nCnt);

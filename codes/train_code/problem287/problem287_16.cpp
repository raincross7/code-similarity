...
int main() {
	...
	if (a == b) {
		...
		int temp = 0;
		for(auto itr : av) {
			if (itr.second > count_c && itr.first != a) {
				temp = itr.first;
				count_c = itr.second;
			}
		}
		int r_a = calc(n, temp, b, v);
	
		count_c = 0;
		for(auto itr : bv) {
			if (itr.second > count_c && itr.first != b) {
				temp = itr.first;
				count_c = itr.second;
			}
		}

		int r_b = calc(n, a, temp, v);

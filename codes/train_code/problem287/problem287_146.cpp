...
int main()
{
    ...
    std::map<long long, long long> odds, evens;
    for (int i = 0; i < n; i++) {
        if (i % 2) {
            odds[v[i]]++;
        } else {
    ...
    std::pair<long long, long long> max_odds, next_odds, max_evens, next_evens;
    max_odds = next_evens = max_evens = next_evens = std::make_pair(0, 0);

    for (auto t : odds) {
        if (t.second > max_odds.second) {
            next_odds = max_odds;
            max_odds = t;
        } else if (t.second > next_odds.second) {
            next_odds = t;
        }
    }
    ...
    for (auto t : evens) {
        if (t.second > max_evens.second) {
            next_evens = max_evens;
            max_evens = t;
        } else if (t.second > next_evens.second) {
            next_evens = t;
        }
    }

    if (max_odds.first == max_evens.first) {
        if (max_odds.second - next_odds.second > max_evens.second - next_evens.second) {
            std::cout << n - max_odds.second - next_evens.second << std::endl;
        } else {
            std::cout << n - next_odds.second - max_evens.second << std::endl;
        }
    } else {
        std::cout << n - max_odds.second - max_evens.second << std::endl;
    }
}

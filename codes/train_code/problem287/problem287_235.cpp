...
vector<PII> f(map<ll,ll> mp){
  vector<PII> ret;
  for(auto pa : mp){
    ll v = pa.first;
    ll c = pa.second;
    ret.push_back(MP(c,v));
  }
...
int main(){
    ...
    if(V0[0].second == V1[0].second){
      debug("wow");
      // 文字が同じ時、どちらかが折れないといけない
      ll keep0 = V0[0].first + V1[1].first;
      ll keep1 = V0[1].first + V1[0].first;
      ll ma = max(keep0, keep1);
      ll ans = N-ma;
      p(ans);
    }
    else{
      ll keep = V0[0].first + V1[0].first;
      ll ans = N-keep;
      p(ans);
    }

...
// 弧度法（radian）から度数法（degree）へ変換
ld to_deg(ld rad){
  return rad * ((ld)(180) / PI);
}

bool judge_tri(ld alpha){
  if ((b * b / 2) * tan(to_rad(90 - alpha)) < x / a) return true;
  else return false;
}

bool judge_daikei(ld alpha){
  if (a * b - (a * a / 2) * tan(to_rad(alpha)) < x / a ) return true;
  else return false;
}
...
bool isOK(ld key, bool tri) {
  // 可否を評価
  // okを適当に大きめの値に設定した場合、配列外参照やコーナーケースに注意

  if ( (tri ? judge_tri(key) : judge_daikei(key)) ){
    return true;
  }
  else{
    return false;
...
ld BinarySearch(bool tri) {
  ...
  // 実数の探索なので、固定回数でループしないと誤差でバグる
  int maxLoop = 50; // 二分探索の場合
  for (int i = 0; i < maxLoop; ++i){
    ld mid = (ok + ng) / 2;

    if (isOK(mid, tri)) ok = mid;
    else ng = mid;
  }
  return ok;
}
...
int main(){
  ...
  ld ans;
  if (a * b / 2 > x / a){
    ans = BinarySearch(true);
  }
  else{
    ans = BinarySearch(false);
  }

...
bool check(ll a , ll b , ll mid)
{
    ...
    if(y >= b)
        y++ ;
   // cout << "x = " << x << " y = " << y << endl ;
    ll init_x = max(1 , x/2 - 5) ;
    ll init_y = ((1 + y) - init_x) ;
    if(init_x < x)
    {
        if(init_y <= b && mid >= b)
            init_y-- ;
    }
    //cout << "mid = " << mid << " init_x = " << init_x << "  init_y = " << init_y << endl ;
    
    for(ll i = 1 ; i < 10 && init_x <= x && init_y >= 1 ; i++ , init_x++ , init_y--)
    {

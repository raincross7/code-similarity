...
int main()
{
  ...
  //main
  ll ans = 0;
  for(ll n=0; n<=(min(N,K)); n++){//宝石を取り出す回数
    for(ll left=0; left<=n; left++){//左から宝石を取り出す回数
      ll tmp = 0;
      vector<ll> v(0);
      
      for(ll i=0; i<left; i++){//left回取り出す
        tmp += V[i];
        v.pb(V[i]);
      }
      for(ll i=0; i<n-left; i++){//n-left回取り出す
        tmp += V[N-1-i];
        v.pb(V[N-1-i]);
      }
      if(v.size() >= 1){
        sort(all(v));
        for(ll i=0; i<(min(K-n,(ll)v.size())); i++){
          if(v[i] < 0){
            tmp -= v[i]; 
          }
        }

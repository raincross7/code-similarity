...
void solve(){
  ...
  FOR(l,0,n+2){
    FOR(r, l+1, n+2){
      if( (l-0) + (n+1-r) > k) continue;
      V<ll> p; ll sum = 0;
      FOR(j,1,l+1) {p.eb( v[j] ); sum += v[j];}
      FOR(j,r,n+1) {p.eb( v[j] ); sum += v[j]; }
      int cnt = 0;
      sort( RALL(p) );
      while( (l-0) + (n+1-r) + cnt <= k ){
        ans = max(sum, ans);
        if( p.size() && p.back()< 0) {sum -= p.back(); p.ppb(); ++cnt;}
        else break;
      }
      //couts(1);couts(l); couts(r); couts(n); coutn(ans);
    }
  }
  ncoutn( ans );
}

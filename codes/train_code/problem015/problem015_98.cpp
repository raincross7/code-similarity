...
int main()
{
    ll n, k;
    cin >> n >> k;
    vector<ll> a(1000, infll);
    for (ll i = 200; i < 200 + n; i++)
    {
        cin >> a[i];
        a[i - n] = a[i];
    }
    ll maxtimes = min(amarinasi(k, (ll)2), n);
    ll ans = 0;
    for (ll i = 0; i <= maxtimes; i++)
    {
        ll range = min(i + (k - i * 2), n);
        for (ll j = 200 - range; j <= 200; j++)
        {
            priority_queue<ll, vector<ll>, greater<ll>> pq;
            ll sum_tmp = 0;
            for (ll k = j; k < j + range; k++)
            {
                if (a[k] == infll)
                {
                    continue;

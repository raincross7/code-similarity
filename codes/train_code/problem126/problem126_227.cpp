...
void smain() {
    ...
    int w, h;
    cin >> w >> h;
    vector<pair<int, pair<bool, int>>> edges;
    fori (i, w) {
        int p;
        cin >> p;
        edges.push_back({p, {0, i}});
    }
    ...
    for (auto e : edges) {
        ...
        if (e.second.first) {
            ans += (ll) cost * wdsu.cnt;
            hdsu.union_sets(i, i + 1);
        } else {
            ans += (ll) cost * hdsu.cnt;
            wdsu.union_sets(i, i + 1);
        }
//        watch(ans);
    }
    cout << ans;

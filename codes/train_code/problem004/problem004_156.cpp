...
signed main() {
  ...
  for(auto c:t){
    index++;
    int prev = max(max(dp[index-1][0],dp[index-1][1]),dp[index-1][2]);
    dp[index][0] = dp[index][1] = dp[index][2] = prev;
    switch(c)
    {
        case 'r':
          dp[index][2] += p;
    ...
    if(index>k){
      ...
      {
        case 'r':
          dp[index][2] += max(dp[index-k][0],dp[index-k][1])-dp[index-k][2];
          break;
        case 's':
          dp[index][0] += max(dp[index-k][2],dp[index-k][1])-dp[index-k][0];
          break;
        case 'p':
          dp[index][1] += max(dp[index-k][2],dp[index-k][0])-dp[index-k][1];
          break;
      }
    }
  }

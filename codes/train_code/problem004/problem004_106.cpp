...
int main(void)
{
	...
	vvt<ll> dp; MakeVVT(Size(T) + 1, 3, dp);
	rep(i, 1, Size(T))
	{
		rep(j, 0, 3)
		{
			if (T[i] == 'r')
			{
				...
				dp[i][2] = max({ dp[i - K][0] + P, dp[i - K][1] + P });
			}
			else if (T[i] == 's')
			{
				if (i - K < 0)
				...
				dp[i][0] = max({ dp[i - K][1] + R, dp[i - K][2] + R });
				dp[i][1] = max({ dp[i - K][0], dp[i - K][2] });
			...
			else
			{
				...
				dp[i][0] = max({ dp[i - K][1], dp[i - K][2] });
				dp[i][1] = max({ dp[i - K][0] + S, dp[i - K][2] + S });
				dp[i][2] = max({ dp[i - K][0], dp[i - K][1] });
			}
		}
	}
	ll ans = 0;
	rep(i, Size(T) - K, Size(T)) ans += max({dp[i][0], dp[i][1], dp[i][2]});
	cout << ans;
	return 0;
}

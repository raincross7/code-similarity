...
void solve(int n, int k, vi t, vi d) {
  int ds = 0;
  int ts = 0;
  auto vdt = zip(d, t);
  RSORT(vdt);
  ...
  rep(i, k) {
    ds += vdt[i].first;
    if(ok[vdt[i].second])rests.push_back(vdt[i].first);
    else {
      ok[vdt[i].second] = true;
      ts++;
    }
  }
  RSORT(rests);
  ...
  rep(i, k, n) {
    if(sz(rests) == 0)break;
    if(ok[vdt[i].second])continue;
    ds += - rests.back() + vdt[i].first;
    ok[vdt[i].second] = true;
    rests.pop_back();
    ts++;
    amax(ans, ds + ts * ts);

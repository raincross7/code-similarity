...
//Library-End
int n,gou,k,ans,ty,now;
pair<int, int> t[100004];
vector<int> ve[100004];
vector<int> shimojimo;
vector<int> shunou;
signed main(){
    cin>>n>>k;
    rep(i, n){
    ...
    rep(i, n-1){
        if (t[i].first!=t[i+1].first) {
            now++;
            shunou.push_back(t[i].second);
        }else{
            shimojimo.push_back(t[i].second);
        }
        
    }
    shunou.push_back(t[n-1].second);
    sort(shimojimo.begin(),shimojimo.end(),greater<int>());
    sort(shunou.begin(),shunou.end(),greater<int>());
    rep(i, (int)shunou.size()-1){
        shunou[i+1]+=shunou[i];
    }
    rep(i, (int)shimojimo.size()-1){
        shimojimo[i+1]+=shimojimo[i];
    }
    
    for (int i=1; i<=min(now,k); i++) {
        if (i-1>=(int)shunou.size()||k-i-1>=(int)shimojimo.size()) {
            continue;
        }
        int karians=i*i+shunou[i-1];
        if(k-i-1>=0)karians+=shimojimo[k-i-1];

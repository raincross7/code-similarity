...
vector<int> ve[100004],su,shunou,typs;
...
signed main(){
    ...
    rep(i, n-1){
        ve[t[i].first].push_back(t[i].second);
        if (t[i].first!=t[i+1].first) {
            ty++;
            typs.push_back(t[i].first);
        }
    }
    typs.push_back(t[n-1].first);
    ve[t[n-1].first].push_back(t[n-1].second);
    for (int i=0; i<(int)typs.size(); i++) {
        for (int j=0; j<(int)ve[typs[i]].size()-1; j++) {
            su.push_back(ve[typs[i]][j]);
        }
        if((int)ve[typs[i]].size()>0){
            shunou.push_back(ve[typs[i]][(int)ve[typs[i]].size()-1]);
        }
    }

...
using namespace std;
	...
	signed main(){ signal(SIGFPE, catch_SIGFPE); signal(SIGSEGV, catch_SIGSEGV); convertedmain(); return 0;	}
...
void solve() {
	...
	rep(i,N){
		...
		if(a[i].size()>1)sort(all(a[i]),greater<ll>());
		c.push_back(a[i][0]);
	}
	...
	priority_queue<ll,vector<ll>,greater<ll>> dlist;
	ll i=0;
	for(;i<min(K,(ll)c.size());i++){
		ans+=c[i];
		dlist.push(c[i]);
	}
	...
	ans+=pow(dlist.size(),2);
	ll various=dlist.size();
	if(K>c.size()){
		while(i<K){
			ans+=q.top();
			dlist.push(q.top());
		...
		for(;i<c.size();i++)q.push(c[i]);
	}

	while(!q.empty()){
		debug(dlist.top(),q.top());
		if(q.top()-dlist.top()-various*various+(various-1)*(various-1)<0){
			break;
		}
		ans+=q.top();
		ans-=dlist.top();

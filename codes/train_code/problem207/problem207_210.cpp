...
namespace IO {
    template <typename TYPE>
        class TableIO {
            public:
                ...
                static vector<vector<TYPE> > MakeTableWithWall(const Int h, const Int w, const TYPE& init); // 壁付き
                // 二次元配列を出力
    ...
    template <typename TYPE>
        vector<vector<TYPE> > TableIO<TYPE>::MakeTable(Int h, Int w) {
            vector<vector<TYPE> > input(h, vector<TYPE>(w));
            for (Int i = 0; i < h; ++i) {
                for (Int j = 0; j < w; ++j) {
                    cin >> input[i][j];
    ...
    template <typename TYPE>
        vector<vector<TYPE> > TableIO<TYPE>::MakeTableWithWall(Int h, Int w, const TYPE& init) {
            vector<vector<TYPE> > input(h + 2, vector<TYPE>(w + 2, init));
            for (Int i = 1; i <= h; ++i) {
                for (Int j = 1; j <= w; ++j) {
                    cin >> input[i][j];
...
int main(void) {
    Int h, w;
    cin >> h >> w;
    vector<vector<char> > v = TableIO<char>::MakeTableWithWall(h, w, '.');
    bool result = true;

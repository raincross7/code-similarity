...
//vv[j] = min({j,tsum-j,v[i],(j-tst)+limst,(ten-j)+limen,vv[j-1]+1});
...
signed main() {
  ...
  rep(i,0,n){
    ...
    repeq(j,tst,ten){
      //vv[j] = min({j,tsum-j,v[i],(j-tst)+limst,(ten-j)+limen,vv[j-1]+1});
      vv[j] = okmax(j,i);
      //vv[j] = min(vv[j-1]+1,vv[j]);
      double lit = j - 0.5;
      /*
      vector<double> vec={lit,tsum-lit,v[i],(lit-tst)+limst,(ten-lit)+limen,vv[j-1]+1};
      sort(ALL(vec));
      */
      double made = okmax(lit,i);
      if(lit > tst && vv[j]==vv[j-1] && made >= vv[j] + 0.48) bi++;

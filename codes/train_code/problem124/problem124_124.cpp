...
double limit(double tim){
    auto d = (lower_bound(all(t), tim) - t.begin());
    double limit = v[d-1];
    FOR(di,d,n+1){
        limit = min(limit,v[di] + abs(tim - t[di]));
    }
    return limit;
}
...
signed main(){
    ...
    for(double tim = 0.0; tim <= t[n]; tim += 0.5){
        psp = sp;
        sp = min(psp+0.5, limit(tim));
        // cerr << tim << " :" << sp << endl; 
        r += (psp+sp) * 0.5 * 0.5;
    }
    printf("%.13f\n", r);

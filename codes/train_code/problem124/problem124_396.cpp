...
int main()
{
  ...
  ll N; cin >> N;
  vector<ll> T(N); vector<ll> Tsum(N+1,0); 
  FOR(i,N){ cin >> T[i]; } FOR(i,N){ Tsum[i+1] = Tsum[i]+T[i]; }
  vector<ll> V(N); ll Vmax = 0;
  FOR(i,N){ cin >> V[i]; Vmax = max(Vmax,V[i]); }
  
  //main
  double def = -1e10;
  vector<vector<double>> dp(2*Tsum[N]+1,vector<double>(2*Vmax+1,def));
  dp[0][0] = 0; ll state = 0;
  for(ll t=1; t<=2*Tsum[N]; t++){
    if(2*Tsum[state+1] < t){
      state ++; 
    }
    for(ll v=0; v<=2*V[state]; v++){
      if(v+1<=2*V[state] && dp[t-1][v] != def){//加速
        dp[t][v+1] = max(dp[t][v+1],dp[t-1][v]+(0.25*v+0.125));  
      }
      if(v-1>=0 && dp[t-1][v] != def){//減速
        dp[t][v-1] = max(dp[t][v-1],dp[t-1][v]+(0.25*v-0.125)); 
      }
      //維持
      if(dp[t-1][v] != def){
        dp[t][v] = max(dp[t][v],dp[t-1][v]+0.25*v); 
      }
    }
  }
  
  double ans = dp[2*Tsum[N]][0];

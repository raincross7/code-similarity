...
int main() {
  ...
  VVI next(60,VI(N,-1));
  VVLL sum(60,VLL(N,0));
  VVLL res(60,VLL(N,RLINF));

  REP(i,N){
    next.at(0).at(i) = P.at(i);
    sum.at(0).at(i) = C.at(i);
    res.at(0).at(i) = C.at(i);
  }
  REP(i,60-1)REP(j,N){
    next.at(i+1).at(j) = next.at(i).at(next.at(i).at(j));
    sum.at(i+1).at(j) = sum.at(i).at(j)+sum.at(i).at(next.at(i).at(j));
    res.at(i+1).at(j) = max(res.at(i).at(j),sum.at(i).at(j)+res.at(i).at(next.at(i).at(j)));
  }

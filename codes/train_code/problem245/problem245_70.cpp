...
int main()
{
  ...
  vector<ll> maxScore(N,-1e18);
  vector<ll> maxScoreLast(N,-1e18);
  
  FOR(i,N){
    ll last = cycle[i]+(K%cycle[i]);
    
    if(K >= last){
      ll tmp = 0; ll next = P[i];
      FOR(k,last){
        tmp += C[next];
        maxScoreLast[i] = max(maxScoreLast[i],tmp);
        next = P[next];
      }
    
      ll scoreCycle = 0; next = P[i];
      FOR(k,cycle[i]){
        scoreCycle += C[next];
        next = P[next];
      }
      ...
      if(scoreCycle > 0){
        maxScore[i] = scoreCycle*((K-last)/cycle[i])+maxScoreLast[i];
      }else{
        maxScore[i] = maxScoreLast[i];
      }

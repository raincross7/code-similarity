        ...
        rep(xj, n - xi - 1) {
            rep(yi, n) {
                rep(yj, n - yi - 1) {
                    ll left = lefts[xi];
                    ll right = rights[xj];
                    ll bottom = bottoms[yi];
                    ll top = tops[yj];

                    // 何点含まれているか
                    int cnt = 0;
                    cnt = sum.get(yi, xi, n - yj, n - xj);
                    //cout << "cnt : " << cnt << " left : " << left << ", right : " << right;
                    //cout << ", bottom : " << bottom << ", top : " << top << ", mes : " << (right - left) * (top - bottom) << endl;

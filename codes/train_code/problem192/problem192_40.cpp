...
int main(){
  ...
  for(int j1=0; j1<N; j1++){
    for(int j2=j1+1; j2<N; j2++){
      ...
      int64_t xmax_cand= max(p1.first, p2.first);
      int64_t xmin_cand= min(p1.first, p2.first);

      if(Kin==0){
        int64_t xmax= xmax_cand;
        int64_t xmin= xmin_cand;
        ...
        for(int i=0; i<N; i++){
          if(i==j1 || i==j2){ continue; }
            ...
            if(ymin <= data.at(i).second && data.at(i).second <= ymax){
              ...
              if(Nin >= Kin){
                int64_t xmax= max(Xs_in_Yrange.back()     , xmax_cand);
                int64_t xmin= min(Xs_in_Yrange.at(Nin-Kin), xmin_cand);

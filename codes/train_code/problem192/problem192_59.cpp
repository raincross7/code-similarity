...
int main() {
    ...
    rep(i, n*n) {
        srep(j, i+1, n*n) {
            ll x_mn = min(nx[i], nx[j]);
            ll x_mx = max(nx[i], nx[j]);
            ll y_mn = min(ny[i], ny[j]);
            ll y_mx = max(ny[i], ny[j]);
            int count = 0;
            rep(k, n) {
                if (x[k] >= x_mn && x[k] <= x_mx && y[k] >= y_mn && y[k] <= y_mx) count++;
            }
            if (count >= K) {
                ans = min(ans, (x_mx - x_mn) * (y_mx - y_mn));
            }

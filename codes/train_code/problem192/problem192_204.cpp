        ...
        for (int j=i+1; j<n; j++) {
            for (int k=0; k<n-1; k++) {
                for (int l=k+1; l<n; l++) {
                    ...
                    xmin=min(x[i].first,x[j].first);
                    xmax=max(x[i].first,x[j].first);
                    ymin=min(x[k].second,x[l].second);
                    ymax=max(x[k].second,x[l].second);
                    rep(k, n){
                        if (x[k].first>=xmin&&x[k].first<=xmax&&x[k].second>=ymin&&x[k].second<=ymax) {
                            cnt++;
                        }
                    }
                    if (cnt>=K) {

...
int main() {
  ...
  ll ans=1e19;
  rep(i,n) cin>>x[i]>>y[i];
  rep(i,n) for(ll j=i+1;j<n;j++) rep(k,n) for(ll l=k+1;l<n;l++){
    ll tmp=0;
    rep(m,n) if(min(x[i],x[j])<=x[m] && x[m]<=max(x[i],x[j]) && min(y[k],y[l])<=y[m] && y[m]<=max(y[k],y[l])) tmp++;
    if(tmp>=K) chmin(ans,abs(x[j]-x[i])*abs(y[l]-y[k]));
  }
  cout<<ans<<endl;
}
...
int main()
{
  ...
  for(int i1=0; i1<N-K+1; i1++){
    for(int i2=i1+K-1; i2<N; i2++){
      for(int j1=0; j1<N-K+1; j1++){
        for(int j2=j1+K-1; j2<N; j2++){
          ll Xmin = ccx.getKey(i1), Xmax = ccx.getKey(i2);
          ll Ymin = ccy.getKey(j1), Ymax = ccy.getKey(j2);
          ll tmp = 0;
          for(int n=0; n<N; n++){
            if(Xmin <= X[n] && X[n] <= Xmax && Ymin <= Y[n] && Y[n] <= Ymax){
              tmp ++; 
            }
          }
          if(tmp >= K){

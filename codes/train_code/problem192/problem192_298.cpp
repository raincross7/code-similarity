    ...
    rep(j,i+1,n){
      rep(k,0,n){
        rep(l,k+1,n){
          ct = 0;
          rep(m,0,n){
            if(into[m].FI >= yoko[i] && into[m].FI <= yoko[j] && into[m].SE >= tate[k] && into[m].SE <= tate[l]) ct++;
          }
          if(ct >= K) ans = min(ans,(yoko[j]-yoko[i])*(tate[l]-tate[k]));
        }
      }
    }
  }

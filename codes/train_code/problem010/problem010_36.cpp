...
//cout<<""<<endl;
int main(){
  ll N;cin>>N;vl A(N);map<ll,ll> mp;rep(i,N){cin>>A[i];mp[A[i]]++;}
  ll a=0,b=0,c=0;
  
  for(auto x:mp){
    if(x.second>=2){a=max(x.first,a);}
  }
  for(auto x:mp){
    if(x.second>=2 && x.first!=a){b=max(x.first,b);}
  }
  for(auto x:mp){
    if(x.second>=4){c=max(x.first,c);}
  }
  cout<<max(a*b,c*c)<<endl;

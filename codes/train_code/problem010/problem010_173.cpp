...
int main() {
	...
	for(auto itr: m) {
		...
		rect.push_back(itr.first);
	}

	sort(rect.begin(), rect.end(), greater<ll>());
	ll rect1 = rect[0];
	ll rect2 = rect[1];

	ll sq_max = *max_element(square.begin(), square.end());

	cout << (max({rect1*rect2, sq_max*sq_max})) << endl;
  return 0;
}

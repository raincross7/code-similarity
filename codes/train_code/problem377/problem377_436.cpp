	...
	while(t--)
	{
		...
		f(0,n-1)
		{
				...
				f2(0,k)
				{
					...
					pre=modadd(pre,dp[i][j],M1);
					if(j<=k)
					{
						dp[i+1][j]=modadd(pre,dp[i+1][j],M1);
						dp[i+1][min(k,in[i+1]+j)+1]=modsub(dp[i+1][min(k,in[i+1]+j)+1],pre,M1);
					}
				}
			//print2D(dp);
		}

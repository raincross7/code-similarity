...
void solve(){
  ...
  FOR(i,2,n+1){
    bool ifprime = true;
    for(auto x: prime){
      if(i % x==0) { ifprime = false; break;}
      if(x*x>i) break;
    }
    if(ifprime){ prime.eb(i);}
  }
  //coutn(prime);
  V< int > cnts( prime.size() );
  FOR(i,2,n+1){
    int i_ = i;
    FOR(j,0,(int)prime.size()){
      while(i_%prime[j]==0){
        i_ /= prime[j];
        cnts[j]++;
      }
  ...
  ll ans = 0;
  int sm[] = {0,2,4,2}, lg[] = {74,24,14,4};
  //FOR(i,0,4) couts(sm[i]); FOR(i,0,4) couts(lg[i]);coutn("");
  {
    ll lgcnt=0;
    for(auto x: cnts) if(x>=lg[0]) lgcnt++;
    //couts(ans);
    ans += lgcnt;
    //couts(ans);coutn(lgcnt);
  }
  FOR(k,1,3){
    ll smcnt=0, lgcnt=0;
    for(auto x: cnts) if(x>=sm[k]) smcnt++;
    for(auto x: cnts) if(x>=lg[k]) lgcnt++;
    smcnt -= 1;
    //couts(ans);
    ans += smcnt*lgcnt;
    //ans += smcnt*lgcnt + lgcnt*(lgcnt-1)/2;
    //ans += (smcnt+lgcnt)*(smcnt+lgcnt-1)/2 - smcnt*(smcnt-1)/2;
    //couts(ans);couts(smcnt);coutn(lgcnt);
  }
  {
    ll smcnt=0, lgcnt=0;
    for(auto x: cnts) if(x>=sm[3]) smcnt++;
    for(auto x: cnts) if(x>=lg[3]) lgcnt++;
    //smcnt -= lgcnt;
    //couts(ans);
    ans += lgcnt*(lgcnt-1)*(smcnt-2)/2;
    //ans += lgcnt*(lgcnt-1)*(lgcnt-2)/6 + lgcnt*(lgcnt-1)/2 * smcnt;
    //ans += (lgcnt+smcnt)*(lgcnt+smcnt-1)*(lgcnt+smcnt-2)/6 ;//- smcnt*(smcnt-1)*(smcnt-2)/6 -lgcnt*smcnt*(smcnt-1)/2;
    //couts(ans);couts(smcnt);coutn(lgcnt);
  }
  //coutn( cnts );
  coutn( ans );

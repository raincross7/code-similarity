...
map<ll,ll> mp;
ll ans;
ll border3,border5,border15,border25,border75;
int main(){
  cin>>n;
  for(ll i=2;i<=n;i++){
    ll copyi=i;
    for(ll j=2;j*j<=copyi;j++){
      while(copyi%j==0){
  ...
  for(ll i=1;i<=n;i++){
    for(ll j=1;j<=n;j++){
      if(mp[i]>=24&&mp[j]>=2&&i!=j)ans++;
    }
  }
  for(ll i=1;i<=n;i++){
    for(ll j=1;j<=n;j++){
      if(mp[i]>=14&&mp[j]>=4&&i!=j)ans++;
    }
  }
  ...
  for(ll i=1;i<n;i++){
    for(ll j=i+1;j<=n;j++){
      for(ll k=1;k<=n;k++){
        if(mp[i]>=4&&mp[j]>=4&&mp[k]>=2&&i!=j&&j!=k&&i!=k){
          ans++;

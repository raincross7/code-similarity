...
int main(){
  ...
  lli ar1[] = {74} , ar2[] = {24, 2}, ar3[] = {14, 4}, ar4[] = {4, 4, 2};

  lli ans = 0;
  for(int i = 1; i < 100; i++) if(mp[i] >= ar1[0]){
    ans++;
  }
  for(int i = 1; i < 100; i++) if(mp[i] >= ar2[0]){
    for(int j = 1; j < 100; j++) if(j!=i && mp[j] >= ar2[1]){
      ans++;
    }
  }
  for(int i = 1; i < 100; i++) if(mp[i] >= ar3[0]){
    for(int j = 1; j < 100; j++) if(j!=i && mp[j] >= ar3[1]){
      ans++;
    }
  }
  for(int i = 1; i < 100; i++) if(mp[i] >= ar4[0]){
    for(int j = i+1; j < 100; j++) if(mp[j] >= ar4[1]){
      for(int k = 1; k < 100; k++) if(i!=k && j!=k && mp[k] >= ar4[2]){
        ans++;
      }
    }
  }

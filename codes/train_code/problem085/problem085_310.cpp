...
signed main() {
  ...
  int seventyFourCount=0;
  for(auto f:factors){
    //cout << f.first << "," << f.second << endl;
    if(f.second>=4)fourCount++;
    if(f.second>=2)twoCount++;
    if(f.second>=14)fourteenCount++;
    if(f.second>=24)twentyFourCount++;
    if(f.second>=74)seventyFourCount++;
  }

  cout << fourCount*(fourCount-1)*(twoCount-2)/2
    + fourteenCount*(fourCount-1)
    + twentyFourCount*(twoCount-1)
    + seventyFourCount << endl;
  return 0;
}

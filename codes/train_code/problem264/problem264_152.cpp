...
ll solve(int N, vi& a) {
    ll INFTY = 100000000000000LL;

    vll rcap(N+1, INFTY);
    rcap[0] = 1-a[0];
    if (rcap[0] < 0) return -1;
    for(int i=1; i<=N; ++i){
        rcap[i] = min(INFTY, rcap[i-1]*2 - a[i]);
        if (rcap[i] < 0) return -1;
    }

    ll width=a[N], ans = width;
    for(int i=N-1; i>=0; --i){
        if (width > rcap[i]*2) return -1;
        if (width > rcap[i]) width = rcap[i];

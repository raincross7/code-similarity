...
void solve(){
  int n; cin >> n;
  V<ll> a(n+1); cin >> a;
  if(n==0 && a[0]==1) {cout << 1; return;}
  V<ll> b(n+1,0);
  V<ll> sum(n+2,0);
  IFOR(i,0,n+1) sum[i] = sum[i+1]+a[i];
  
  b[0] = 1; ll ret = a[0] + b[0];
  FOR(i,1,n+1){
    b[i] = min<ll>(b[i-1]*2-a[i], sum[i+1]);
    if(b[i]<0) {cout << -1; return;}
    ret += a[i]+b[i];
  }
  if(a[0]+b[0]>1) {cout<<-1; return;}
  cout << ret;
  //coutn(a);
  //coutn(b);

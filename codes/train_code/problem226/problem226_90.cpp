...
void _cout(){ cout << "\n" << flush; } template <class Head, class... Tail> void _cout(Head&& head, Tail&&... tail){ cout << head; _cout(forward<Tail>(tail)...); }
...
int main(void){
    ...
    pair<int, string> lb, ub;
    {
        Cout(0);
        Cin(string, s);
        if(s == "Vacant") return 0;
        
        lb.first = 0, ub.first = n;
        lb.second = ub.second = s;
    }

    while(true){
        int m = (lb.first + ub.first) / 2;
        Cout(m);
        Cin(string, s);
        if(s == "Vacant") return 0;
        // 差が偶数 & 異性 または 差が奇数 & 同性
        if(((m - lb.first) % 2 == 0) ^ (s == lb.second)) ub.first = m, ub.second = s;
        else lb.first = m, lb.second = s;
    }
}

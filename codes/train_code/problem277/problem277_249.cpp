...
bool used[200020];
bool dp[100005];
ll A,B,C,N,K,M,Q,T,H;
double dA,dB,dC,dN,dK,dM,dQ,dT,dH;
...
int main()
{
	...
	long long min,min1;
    long long wk,wk1;
    max=max1=min=min1=wk=wk1=sum=ans=cnt=cnt1=flg1=flg2=0;
	double dwk,dwk1;
	dwk=dwk1=0;
    ...
    map<ll,ll>mp[N];
    //priority_queue<long long, vector<long long>, greater<long long> > que[N];
	rep(i,N){
        rep(j,s[i].size()){
            mp[i][s[i][j]-'a'+1]++;
        }

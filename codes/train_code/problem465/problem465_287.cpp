...
bool dpx[4010][16000+10];
    bool dpy[4010][16000+10];
int main() {
    cin.tie(0);
    ...
    REP(i,4010)REP(j,16000+10) dpx[i][j] = false;
    REP(i,4010)REP(j,16000+10) dpy[i][j] = false;
    ...
    for(int i=0; i<vx.size(); i++){
        for(int j=0; j<=16000; j++){
            if(vx[i] < 0){
                ...
                if(j-vx[i]>=0)dpx[i+1][j] |= dpx[i][j-vx[i]];
                if(j+vx[i]<=16000)dpx[i+1][j] |= dpx[i][j+vx[i]];
            }
       }
    }
    ...
    for(int i=0; i<vy.size(); i++){
        for(int j=0; j<=16000; j++){
            ...
            if(j+vy[i]<=16000)dpy[i+1][j] |= dpy[i][j+vy[i]];
        }
    }
    ...
    if(dpx[vx.size()][x] && dpy[vy.size()][y]){
        cout << "Yes" << endl;
    }else{
        cout << "No" << endl;
    }

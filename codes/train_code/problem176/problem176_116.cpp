...
int main() {
    ...
    ll ret = 0;
    for(ll one = '0'; one <= '9'; one++) {
        for(ll two = '0'; two <= '9'; two++) {
            for(ll three = '0'; three <= '9'; three++) {
                bool flag_one = false;
                bool flag_two = false;
                bool flag_three = false;
                for(ll i = 0; i < S.size(); i++) {
                    if(flag_one == false && S[i] == one) {
                        flag_one = true;
                        continue;
                    } else if(flag_one == true && flag_two == false && S[i] == two) {
                        flag_two = true;
                        continue;
                    } else if(flag_one == true && flag_two == true && flag_three == false && S[i] == three) {
                        ret++;
                        break;
                    }
                } 

...
int main(){
    ...
    rep(i,10)rep(j,10)rep(k,10){
        //(i,j,k)
        if(lower_bound(all(stock[i]),0)==stock[i].ed){
            continue;
        }
        else{
            ll tmpi=*(lower_bound(all(stock[i]),0));
            if(upper_bound(all(stock[j]),tmpi)==stock[j].ed){
                continue;
            }
            else{
                ll tmpj=*(upper_bound(all(stock[j]),tmpi));
                if(upper_bound(all(stock[k]),tmpj)!=stock[k].ed){
                    //cout<<i<<j<<k<<endl;
                    ans++;
                }
            }

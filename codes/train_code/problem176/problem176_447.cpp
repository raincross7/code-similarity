	...
	struct NyaaString
	{
		...
		*/
		static std::string LtoS(long long n, long long size)
		{
			...
			{
				for (long long i = 0; i < size - (long long)ret.size(); i++) zero += "0";
			}
			return zero + ret;
		}
	};
...
int main(void)
{
	...
	rep(i, 0, 1000)
	{
		auto i1 = S.find(NyaaString::LtoS(i, 3)[0]);
		if (i1 == string::npos) continue;
		auto i2 = S.find(NyaaString::LtoS(i, 3)[1], i1+1);
		if (i2 == string::npos) continue;
		auto i3 = S.find(NyaaString::LtoS(i, 3)[2], i2+1);
		if (i3 != string::npos) ans++;
	}
	cout << ans;

...
int main(void)
{
	...
	rep(i, 0, 1000)
	{
		...
		if (Size(test) == 1) test = "00" + test;
		else if (Size(test) == 2) test = "0" + test;
		auto it1 = S.find(test[0], 0);
		if (it1 == string::npos) continue;
		auto it2 = S.find(test[1], it1 + 1);
		if (it2 == string::npos) continue;
		auto it3 = S.find(test[2], it2 + 1);
		if (it3 == string::npos) continue;
		++ans;
	}
	cout << ans;
	return 0;

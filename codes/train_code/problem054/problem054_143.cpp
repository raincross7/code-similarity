...
string calc(string s)
{
    ...
    string t1=calc(s.substr(0,sz(s)/2));
    string t2=calc(s.substr(sz(s)/2,sz(s)));
    if(t1<t2)
    {
        return t1+t2;
    }
...
void solve()
{
    ...
    cin>>a>>b;
    double ans1=(a*100)/8;
    double ans2=(b*100)/10;
    double k1=0.08*ans1;
    double k2=0.1*ans1;
    double k3=0.08*ans2;
    double k4=0.1*ans2;
    double ans=1e18;
    if(floor(0.08*ans1)==a&&floor(0.1*ans1)==b)
    {
        ans=min(ans,ans1);
    }   
    else if(floor(0.08*ans2)==a&&floor(0.1*ans2)==b) 
    {
        ans=min(ans,ans2);

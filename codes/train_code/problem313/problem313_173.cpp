...
int main()
{
  ...
  ll N, M; cin >> N >> M;
  vector<int> p(N); FOR(i,N){ cin >> p[i]; p[i]--; }
  vector<pair<int,int>> vp(M);
  FOR(i,M){ cin >> vp[i].fi >> vp[i].se; vp[i].fi--; vp[i].se--;}
  vector<int> now(N); FOR(i,N){ now[p[i]] = i;}  
    
  //main
  ll ans = 0; 
  UnionFind uf(N);
  FOR(i,M){ uf.unite(vp[i].fi,vp[i].se); }
  FOR(i,N){ 
    //cout << now[p[i]] << p[i] << uf.isUnited(now[p[i]],p[i]) << endl;
    if(uf.isUnited(now[p[i]],p[i])){
      ans ++;    
    }
  }
  // output

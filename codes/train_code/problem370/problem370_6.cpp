...
void dfs(ll key){
  rep(i,graph[key].size()){
    if(C[graph[key][i].to] && DIS[graph[key][i].to]!=DIS[key]+graph[key][i].cost){
      Y=1;
    }
    else if(C[graph[key][i].to]==0){
      C[graph[key][i].to]=1;
      DIS[graph[key][i].to]=DIS[key]+graph[key][i].cost;
      dfs(graph[key][i].to);
    }
  }
}

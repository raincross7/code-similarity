...
int main() {
    ...
    sort(ALL(p), [](const vector<ll> &alpha, const vector<ll> &beta) {
        return alpha[0] < beta[0];
    });
    bool can = true;
    FOR(i, 1, N + 1) {
        ll diff = p[i][0] - p[i - 1][0];
        ll diff_met=abs(p[i][1] - p[i - 1][1]) + abs(p[i][2] - p[i - 1][2]);
        if (diff % 2 == 0) {
            if (diff_met % 2 == 0 && diff >= diff_met) {
                continue;
            } else {
                can = false;
                break;

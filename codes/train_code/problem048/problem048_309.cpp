...
struct BIT{
    ...
    //key以上の値
            if(key<=0){
                ...
                for(int length=r;length>0;length=length>>1){//長さlengthは1段下るごとに半分に
...
struct BIT2D{
    ...
    void add(int h,int w,T x){//(h,w)にxを足す0(1)  (区間サイズnの場合O(N))
    ...
    T partsum(int h,int w,int hh,int ww){//[h,hh][w,ww]の総和O(4*log(max(h,w,hh,ww)))
        //前処理でsumを計算しておけば計算量減らせるかも
        return sum(hh,ww)+sum(h-1,w-1)-sum(hh,w-1)-sum(h-1,ww);
    }
};
int main(){
    //計算量は0(2*N+3*NlogN+2N(logN)^2)
    //O(1.6*10^8)なので間に合う
    int N,K;
    cin>>N>>K;
    vector<int> A(N);

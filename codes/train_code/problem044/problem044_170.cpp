...
int32_t main()
{
	...
	int n;
	cin >> n;
	vi h(n, 0); zip(i, 0, n)  cin >> h[i];
	int ans = 0;
	//better O(n) approach for this problem previos approach is brute force very bad.....
	...
	zip(i, 1, n)
	{
		...
		if (x > 0) //because agar element pichle wale sae choa hai to pichla wala sambhaal lega varna apan ko eventually difference to add karna hoga
		{
			ans += x;
		}
	}
	...
	// 			y = max(h[k], y);
	// 		}
	// 		if ((i == 0 || h[i - 1] == 0) && (j == n - 1 || h[j + 1] == 0) && y > 0)
	// 		{
	// 			zip(k, i, j + 1)
	// 			{
	// 				h[k] -= x;

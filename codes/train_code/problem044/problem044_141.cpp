...
void dfs(ll cur, bool flag3, bool flag5, bool flag7) {

	if (cur > N)return;

	if (flag3 && flag5 && flag7) ans++;

	dfs(cur * 10 + 3, true, flag5, flag7);

	dfs(cur * 10 + 5, flag3, true, flag7);

	dfs(cur * 10 + 7, flag3, flag5, true);
...
int main()
{
	...
	while (true) {
		...
		if (*max_element(data.begin(), data.end()) == 0) {
			break;
		}
		int i = 0;
		while (i < n) {

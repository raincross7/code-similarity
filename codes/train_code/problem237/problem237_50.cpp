...
int main(){
  ...
  vector<ll> pm = {-1, 1};
  // x, y, z について、プラスにするかマイナスにするかを固定して貪欲
  ll ans = 0;
  rep(xpm, 2) rep(ypm, 2) rep(zpm, 2){
    vector<ll> scores(N);
    rep(i, N) scores[i] = xs[i] * pm[xpm] + ys[i] * pm[ypm] + zs[i] * pm[zpm];
    sort(begin_end(scores));
    reverse(begin_end(scores));
    
    ll tmp = 0;
    rep(i, M) tmp += scores[i];
    chmax(ans, tmp);
  }

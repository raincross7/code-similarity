...
auto NyaSort = [](const Cake& l, const Cake& r)
...
int main()
{
	...
	rep(i, 0, Pow2(3))
	{
		...
		each(j, e, test)
		{
			e.a = (NyaBit::Test(i, 0) == 1) ? -cake[j].a : cake[j].a;
			e.b = (NyaBit::Test(i, 1) == 1) ? -cake[j].b : cake[j].b;
			e.c = (NyaBit::Test(i, 2) == 1) ? -cake[j].c : cake[j].c;
		}
		sort(all(test), NyaSort);
		rep(j, 0, M) sum[i] += (test[j].a + test[j].b + test[j].c);
	}

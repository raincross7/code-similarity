...
int main()
{
    ...
    std::vector<long long> as(a);
    for (int i = 0; i < a; i++) {
        std::cin >> as[i];
    }

    std::vector<long long> bs(b);
    for (int i = 0; i < b; i++) {
        std::cin >> bs[i];
    }

    long long ans = *std::min_element(as.begin(), as.end()) + *std::min_element(bs.begin(), bs.end());

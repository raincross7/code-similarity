...
ll solve(ll in, ll small) {
    if(in == n) return 1;
    if(dp[in][small] != -1) return dp[in][small];

    ll d = s[in] - '0', res = 0;

    if(d == 0) {
        res = solve(in+1, small) % mod;
        if(small) {
            res = (res + 2*solve(in+1, 1)) % mod;
        }
    }
    else {
        res = solve(in+1, 1) % mod;
        res = (res + 2*solve(in+1, small)) % mod;
    }

    return dp[in][small] = res;
}

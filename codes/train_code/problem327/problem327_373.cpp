...
#define outD(fpv) cout << setprecision(20) << fpv;
...
template<typename Type> inline void unique(Type& arr) { arr.erase(unique(arr.begin(), arr.end()), arr.end()); }
...
int main()
{
    ...
    cin >> W >> H >> x >> y;
    bool c = (W == x * 2) && (H  == y * 2);
    printf("%.10lf %d", double(W) * double(H) / 2.0, c ? 1 : 0);
    //
    return 0;
}
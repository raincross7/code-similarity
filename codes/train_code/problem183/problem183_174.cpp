...
void dfs(Veci* nodes,Veci* colors,int which, int n,int donotuse, int donotusemod)
{
	int color = (donotuse + 1)%donotusemod;
	repi(nodes[which].n)
	{
		colors[which].data[i] = color;
		dfs(nodes, colors, nodes[which].data[i], n, color, donotusemod);
		color++;
	}
}

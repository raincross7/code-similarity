...
int main()
{
    ...
    while (!pq.empty())
    {
        ...
        ll j = get<2>(tmp);
        ll k = get<3>(tmp);
        if (i + 1 < x && !set.count(make_tuple(i + 1, j, k)))
        {
            pq.push(make_tuple(a[i + 1] + b[j] + c[k], i + 1, j, k));
            set.insert(make_tuple(i + 1, j, k));
        }
        if (j + 1 < y && !set.count(make_tuple(i, j + 1, k)))
        {
            pq.push(make_tuple(a[i] + b[j + 1] + c[k], i, j + 1, k));
            set.insert(make_tuple(i, j + 1, k));
        }
        if (k + 1 < z && !set.count(make_tuple(i, j, k + 1)))
        {
            pq.push(make_tuple(a[i] + b[j] + c[k + 1], i, j, k + 1));
            set.insert(make_tuple(i, j, k + 1));
        }
    }
    for (auto f : ans)
    {

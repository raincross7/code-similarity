...
int main() {
	...
	sort(C.begin(),C.end(),std::greater<SINT64>());
	sort(D.begin(),D.end(),std::greater<SINT64>());

	SINT64 d = MIN(3005,D.size());

	rep(i,0,c) {
		rep(j,0,d) {
			E.emplace_back(C[i]+D[j]);
		}
	}
	sort(E.begin(),E.end(),std::greater<SINT64>());
	rep(i,0,M) {
		put(E[i]);
	}
	return 0;

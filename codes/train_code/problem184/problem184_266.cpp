...
struct cake
{
  LL x;
  int i,j,k;
  cake(LL a,int b,int c,int d):x(a),i(b),j(c),k(d){}
};
...
int main() {
  ...
  SORTR(b);
  SORTR(c);
  vector<VVB> old(x,VVB(y,VB(z,true)));
  old.at(0).at(0).at(0) = false;
  auto f = [](cake &A,cake &B){return A.x < B.x;};
  priority_queue<cake,vector<cake>,decltype(f)> pq{f};
  pq.emplace(a.at(0)+b.at(0)+c.at(0),0,0,0);
  REP(i,k){
    cake now = pq.top(); pq.pop();
    COUT(now.x);
    int s=now.i,t=now.j,u=now.k;
    if(s+1 < x){
      if(old.at(s+1).at(t).at(u)){
        pq.emplace(a.at(s+1)+b.at(t)+c.at(u),s+1,t,u);
        old.at(s+1).at(t).at(u) = false;
      }
    }
    if(t+1 < y){
      if(old.at(s).at(t+1).at(u)){
        pq.emplace(a.at(s)+b.at(t+1)+c.at(u),s,t+1,u);
        old.at(s).at(t+1).at(u) = false;
      }
    }
    if(u+1 < z){
      if(old.at(s).at(t).at(u+1)){
        pq.emplace(a.at(s)+b.at(t)+c.at(u+1),s,t,u+1);
        old.at(s).at(t).at(u+1) = false;
      }
    }
  }
  return 0;

...
ll num(ll n){
    ...
    else if (vec[n] != 0)return vec[n];
    else return vec[n] = 3 + 2 * num(n - 1);
}
ll solve(ll n,ll m){//Pの数
    if(n == 0){
        if(m == 0)return 0;
        else return 1;
    }
        ...
        if(m <= 1)return 0;
        else if(m <= 1 + vec[n - 1])return solve(n - 1,m - 1);
        else if(m == 2 + vec[n - 1])return 1 + solve(n - 1,vec[n - 1]);
        else if(m != vec[n])return 1 + solve(n - 1,vec[n - 1]) + solve(n - 1,m - vec[n - 1] - 2);
        else{
            return 1 + 2 * solve(n - 1,vec[n - 1]);
        }
    }
}
int main(){

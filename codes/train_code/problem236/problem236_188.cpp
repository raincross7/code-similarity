...
ll dfs(ll level, ll under){
    if(level==0) return 1;
    ll len = (1LL<<(level+1))-3;
    ll beef = (1LL<<level)-1;

    if(under==1) return 0;
    else if(under<=len+1) return dfs(level-1,under-1);
    else if(under==len+2) return beef+1;
    else if(under<=2*len+2) return beef+1+dfs(level-1,under-len-2);
    else return beef*2+1;
}

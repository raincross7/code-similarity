...
int main(){
  ...
  //       パティを食べて i+=具材数[Lv.k]
  //       if (i>=具材数[Lv.N]) 完全終了
  //       else Pを1つ食べる, i++, continue;
  //     else k--, Bを1つ食べる, i++, continue;
  while (0 < X){ // X==0になったら食べ終わり
    if (X >= sum_p[k].first){
      X -= sum_p[k].first;
      ans += sum_p[k].second;
      if (X==0) break;
        ...
        ans++;
        X--;
        // ここでX==0になる場合もあるが、自動でwhile抜けるからok
      }
    }
    else{
      X--;

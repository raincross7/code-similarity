...
ll rec(ll n, ll x)
{
        ...
        else if (x <= 1 + ls[n-1]) return rec(n-1, x-1);
        else return ps[n-1] + 1 + rec(n-1, x-2-ls[n-1]);

        // else if (1 <= x and x <= 1 + ls[n-1]) return rec(n-1, x-1);

        // else if (x ==  2 + ls[n-1] ) return 1 + ps[n-1] + rec(n-1, x - 2 - ls[n-1]);

        // else if (2 + ls[n-1] <= x and x <= 2 + 2 * ls[n-1]) return 2*ps[n-1] + rec(n-1, x - 2 - ls[n-1]);

        // else return rec(n-1, x - 2 - 2 * ls[n-1]);
}

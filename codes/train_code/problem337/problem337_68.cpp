...
#define SORT(a) sort(all(a), [](int x, int y){return x > y;})
...
signed main()
{
    ...
    int ans = r*g*b; //cout << ans << '\n';
    rngi(i, n) rngi(j, n) if(i < j){
        int k = 2 * j - i; if(k < j || 0 > k || k >= n)  continue;
        string p; p.pb(s[i]); p.pb(s[j]); p.pb(s[k]);
        if(count(all(p), 'R')  && count(all(p), 'G') && count(all(p), 'B'))
            ans--;
    }
    cout << ans << '\n';
}

...
int main() {
	...
	for (int i = 1; i <= w - 1; i++) {
		long long int s1 = i * h;
		long long int s2 = (w - i) * (h % 2 + h / 2);
		long long int s3 = (w - i) * (h / 2);
		long long int mins = min(s1, min(s2, s3));
		long long int maxs = max(s1, max(s2, s3));
		ans = min(ans, maxs - mins);
	}
	for (int i = 1; i <= h - 1; i++) {
		long long int s1 = i * w;
		long long int s2 = (h - i) * (w % 2 + w / 2);
		long long int s3 = (h - i) * (w / 2);
		long long int mins = min(s1, min(s2, s3));
		long long int maxs = max(s1, max(s2, s3));
		ans = min(ans, maxs - mins);
	}

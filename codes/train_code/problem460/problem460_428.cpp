...
Pll_ll twoDiv(ll h, ll w){
    ll W = w/2;
    return Pll_ll(h*W, h*(w-W));
}

ll threeDiv(ll h, ll w){
    ll ret = h * w;
    FOR(i, 1, h){
        ll S = i * w;
        VLL A(3);
        A[0] = S;
        Pll_ll p = twoDiv(h-i, w);
        A[1] = p.first; A[2] = p.second;
        SORT(A);
        if(A[0]) chmin(ret, A[2]-A[0]);
        A[0] = S;
        p = twoDiv(w, h-i);
        A[1] = p.first; A[2] = p.second;
        SORT(A);
...
int main(){
    ll H, W; cin >> H >> W;
    cout << min(threeDiv(H, W), threeDiv(W, H)) << endl;
    return 0;
}

...
void solve(){
  vector<pair<double, pair<double, double>>> kakudo(2 * N);
  rep(i, N){
    kakudo[i].f = atan2(y[i], x[i]);
    kakudo[i].s.f = y[i];
    kakudo[i].s.s = x[i];
    kakudo[i + N] = kakudo[i];
    kakudo[i + N].f += M_PI * 2;
  ...
  sort(kakudo.begin(), kakudo.end());
  double ans = 0;
  for(ll s = 0; s < N; s++){
    double Y = kakudo[s].s.f;
    double X = kakudo[s].s.s;
    ans = max(ans, d(Y, X));
    for(ll t = s + 1; t < s + N; t++){
      Y += kakudo[t].s.f;
      X += kakudo[t].s.s;
      ans = max(ans, d(Y, X));
    }
  }
  cout << fixed << setprecision(40) << ans << endl;
}

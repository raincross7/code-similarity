...
typedef map<ll, ll, greater<ll>> Chiz;

ll n, A[maxn];

bool next_chiz(Chiz &s, ll m, ll k) {
    ll p = m - 1;
    while (p > 0 && s[p] == k - 1) s.erase(p--);
    if (s[p] == k - 1) return false;
    else s[p]++;
    return true;
}
...
bool check(ll k) {
    Chiz x;
    fori (i, n - 1) {
        while (!x.empty() && x.begin()->F >= A[i + 1]) x.erase(x.begin());
        if (A[i + 1] <= A[i] && !next_chiz(x, A[i + 1], k)) return false;
    }
    return true;
}
...
void MAIN() {
    ...
    fori (i, n) cin >> A[i];
    bool one = true;
    fori (i, n - 1) if (A[i] >= A[i + 1]) one = false;
    if (one) {
        cout << "1\n";
        return;
    }

...
bool chck(int mx){
	vii str;
	str.push_back(ii(0,0));
	str.push_back(ii(a[1],1));
	for(int i=2;i<=n;i++){
		vii::iterator it=str.end();
		it--;
		if(a[i]>it->F){
			if(it->S==1) str.erase(it);
			str.push_back(ii(a[i],1));
		}else{
			vii::iterator it0=lower_bound(str.begin(),str.end(),ii(a[i],0));
			if(it0->S==mx){
				vii::iterator it1=it0;
				it1--;
				...
				it0=it1;--it0;
				str.erase(it1);
				if(t1.F-it0->F>1) str.push_back(ii(t1.F-1,t1.S));
				else if(it0->S==t1.S+1) str.erase(it0);
				str.push_back(ii(t1.F,t1.S+1));
				str.push_back(ii(a[i],1));
			}else{
				vii::iterator it1=it0;
				++it0;
				...
				it0=it1;it0--;
				str.erase(it1);
				if(a[i]-it0->F>1) str.push_back(ii(a[i]-1,t1.S));
				else if(t1.S+1==it0->S) str.erase(it0);
				str.push_back(ii(a[i],t1.S+1));
			}
		}
	}
	return 1;

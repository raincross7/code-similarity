...
vector<int> hotels; int r[MX][31];
int main() {
   //setIO("test"); 
   setIO();
   int N; re(N);
   fax(i,N){
    int x; re(x); x--; hotels.pb(x);
   }
   int L,Q; re(L,Q);
   fax(i,N){
    fax(j,20){
   ...
   fax(i,N){
    //compute r[i][0];
    int lo = 0; int hi = N-1; int ind = 0;
    while(lo<=hi){
      int m = lo + (hi-lo)/2;
      if(hotels[m]<=hotels[i]+L){
        lo = m+1;
        ind = m;
      }
      else{
   ...
   fax(i,Q){
    ...
    while(lo <= hi){
      int days = lo + (hi-lo)/2; bool works = false;
      int pos = ai;
      // cout << "current day: " << days << "\n";
      faxd(j,20){
        // cout << "position " << pos << "\n";
        if(days & (1<<j)){
          if(r[pos][j] == -1){

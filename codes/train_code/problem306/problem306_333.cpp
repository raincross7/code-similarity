...
int main(){
  ...
  int l;
  cin >> l;
  vector<vector<int>> next(n,vector<int>(20));
  for(int i=0;i<n;i++){
    next.at(i).at(0) = upper_bound(x.begin(),x.end(),l+x.at(i)) - x.begin() - 1;
  }
  for(int i=1;i<20;i++){
    for(int j=0;j<n;j++){
      next.at(j).at(i) = next.at(next.at(j).at(i-1)).at(i-1);
    }
  }
  int q;
  cin >> q;

...
int main(){
  ll left = 0, right = pow_long(10, 9);
  while(right - left > 1){
    ll mid = (left + right) / 2;
    ll q1, q2;
    char res1, res2;
    q1 = mid;
    q2 = mid * pow_long(10, 9);
    cout << '?' << " " << q1 << endl;
    cin >> res1;
    cout << '?' << " " << q2 << endl;
    cin >> res2;
    if(res1 == 'Y' && res2 == 'Y') right = mid;
    if(res1 == 'Y' && res2 == 'N') left = mid;
    if(res1 == 'N' && res2 == 'Y') left = mid;
    if(res1 == 'N' && res2 == 'N') right = mid;
  }
  cout << '!' << " " << right << endl;
}

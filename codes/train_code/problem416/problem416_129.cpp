...
bool QUERY(const int v) {
	OUT("? ")OUT(v)ENDL;
	++cnt;
	if (debug) {
		bool f = false;
		f |= v <= N && std::to_string(v) <= std::to_string(N);
		f |= v > N && std::to_string(v) > std::to_string(N);
		SHOW(f);
		return f;
	}

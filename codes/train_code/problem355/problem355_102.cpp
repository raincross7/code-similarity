...
int main() {
	...
	vector<ll> pointSum(D);
	for (ll i = 0; i < D; i++) {
		pointSum[i] = (i + 1) * 100 * p[i] + c[i];
	}
	...
	do {
		//v[0]�`v[D - 1]��v�̗v�f����ёւ������̂������Ă���
		ll count = 0;
		ll point = 0;

		for (auto i : v) {
			if (point + pointSum[i] < G) {
				point += pointSum[i];
				count += p[i];
			}
			else {
...
int main() {
	...
	for (ll i = 1; i < N; i++) {
		ll Kx = x / T[i] + (x % T[i] > 0 ? 1 : 0);
		ll Ky = y / A[i] + (y % A[i] > 0 ? 1 : 0);
		ll K = max(Kx, Ky);
		x = T[i] * K;
		y = A[i] * K;
	}
...
int main() {
	...
	double a = (-1.0 + sqrt(1.0 - (-2.0 * N * 4.0))) / 2.0;
	ll x = (ll)round(a);
	...
	vector<vector<ll>> result(x + 1, vector<ll>(x, 0));
	ll row = 0;
	ll col = 0;
	ll next = 1;
	while (true) {
		...
		ll val = next;
		for (ll j = row + 1; j < x + 1; j++) {
			result[j][col] = val;
			val++;
		}

		val = next + 1;
		for (ll i = col + 1; i < x; i++) {
			result[row][i] = val;
			val++;
		}
...
//AtCoder Beginner Contest 099-D
/*
int main() {
	ll N;
	scanf("%lld", &N);
	...
	vector<vector<ll>> count(3, vector<ll>(C, 0));
	for (ll j = 0; j < N; j++) {
		for (ll i = 0; i < N; i++) {
			count[(i + j) % 3][c[j][i] - 1]++;
		}
	...
	for (ll iGroup = 0; iGroup < 3; iGroup++) {
		for (ll iDstColor = 0; iDstColor < C; iDstColor++) {
			for (ll iSrcColor = 0; iSrcColor < C; iSrcColor++) {
				cost[iGroup][iDstColor] += D[iSrcColor][iDstColor] * count[iGroup][iSrcColor];
			}
		}
	}
	...
	while (true) {
		...
		if (dstColor[0] == C - 1 && dstColor[1] == C - 1 && dstColor[2] == C - 1) {
			break;
		}

		if (dstColor[0] == dstColor[1] || dstColor[0] == dstColor[2] || dstColor[1] == dstColor[2]) {
			continue;
		}

		ll totalCost = cost[0][dstColor[0]] + cost[1][dstColor[1]] + cost[2][dstColor[2]];

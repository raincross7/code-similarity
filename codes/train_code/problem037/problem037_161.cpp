...
int main()
{
	...
	int32_t max_hpoint = 0;
	for (int32_t i = 0; i < n; i++) {
		cin >> v[i].first;
		cin >> v[i].second;
		
		if (v[i].first < max_hpoint) { continue; }
		max_hpoint = v[i].first;
	}

	for (size_t j = 0; j < v.size(); j++) {
		if (dp[v[j].first]) {
			dp[v[j].first] = _min(v[j].second, dp[v[j].first]);
			continue;
		}
	...
	for (int32_t i = 0; i <= h; i++) {
		...
		for (size_t j = 0; j < v.size(); j++) {
			if (dp[i + v[j].first]) {
				dp[i + v[j].first] = _min(dp[i] + v[j].second, dp[i + v[j].first]);
				continue;
			}
	...
	for (int32_t i = h; i < h+max_hpoint; i++) {
		if (!dp[i]) { continue; }
		result = _min(result, dp[i]);
	}

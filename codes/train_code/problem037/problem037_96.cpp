...
int main(){
  ...
  // DP[i] := 体力を i 減らすために最低必要な魔力
  // 体力をN以上減らせる場合もあるが、それはN減らすのと同じなので同一視する
  vector<ll> DP(H+1, LINF);
  DP[0] = 0;

  // まずは単体の魔法を使う場合
  // chmin(DP[0]~DP[min(H, 攻撃力)], 必要魔力)
  rep(i, N){
    ll a = il();
    ll b = il();
    chmin(DP[min(H, a)], b);
  ...
  // 単体の魔法を使う場合（２）
  // DP[H] から見ていって、その時点の最大で一つ前のDP[]をchminしていく
  // 大は小を兼ねるため
  for (ll i=H; 1 <= i; i--){
    chmin(DP[i-1], DP[i]);
  }
  ...
  // 遷移
  // DP[2~H]を下から更新（rep(j:0~i-1){chmin(DP[i], より小さいiのDP2つの組合せ})
  REP(i, 2, H+1){
    rep(j, i){
      if (j > i-j) break;
      chmin(DP[i], ( DP[j] > LINF - DP[i-j] ? LINF : DP[j]+DP[i-j] ) );
    }
  }

...
/*繰り上げ除算*/ll inc(ll x,ll y){ll ans=x/y;if(x%y!=0)ans++;return ans;}
...
int main(){
 ...
 sc(h);sc(n);
 llpvec pair(n);
 ll a,b,dp[30000];//[i番目の魔法までを使って][減る体力]=消耗する魔力の最小

 rep(j,0,30000){dp[j]=llpow(10,8)+10;}
 rep(i,0,n){
  sc(a);sc(b);pair[i]={b,a};
  dp[a]=b;
 } 
  
 ll cnt=0;  
 ll ans=inc(h,pair[0].second)*pair[0].first;
 dp[0]=0;
 rep(i,0,h){
  rep(j,0,n){
   dp[i+pair[j].second]=min(dp[i]+pair[j].first,dp[i+pair[j].second]);

  }
 }
 rep(i,h,30000)ans=min(ans,dp[i]);
 pr(ans); 

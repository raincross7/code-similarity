...
int main(int argc, char const *argv[])
{
    ...
    // pairwise
    // どの組みでもGCD が 1。つまり、どの A_i も素因数を共有しない。
    set<ll> list_num;
    bool not_pairwise = false;
    ll lim = pow(10, 6);
    rep(i, N){
        Vec_Pair tmp = prime_factorization(A[i], lim);
        for(ll_ll p: tmp){
            if(list_num.find(p.first) == list_num.end()){
                list_num.insert(p.first);
            }else{
                not_pairwise = true;
            }
        }
        if(not_pairwise)    break;
    }
    if(!not_pairwise){
    ...
    // setwise
    // 順番にGCD 求めて 1 になったら setwise
    ll gcd_num = GCD(A[0], A[1]);
    FOR(i, 2, N){

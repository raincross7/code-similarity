...
int main() {
	...
	rep(i,0,N) {
		SINT64 a;
		cin >> a;
		data.emplace_back(Pll(a,i));
	}
	sort(data.begin(),data.end(),std::greater<Pll>());
	...
	rep(i,0,N) {
		SINT64 r1,r2,l1,l2;
		SINT64 posi = data[i].S;
		r1 = N;
		r2 = N;
		l1 = -1;
		l2 = -1;
		st.insert(posi);
		auto it = st.find(posi);
		if (it != st.begin()) {
			it--;
			l1 = *it;
			if (it != st.begin()) {
...
//		puts(r1);puts(r2); puts(l1); put(l2);
		ans += ((r2-r1)*l1 + (l2-l1)*r1)*data[i].F;
	}

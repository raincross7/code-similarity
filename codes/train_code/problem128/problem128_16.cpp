...
int main(void)
{
	...
	ll w = 1;
	vvt<ll> wgrid; MakeVVT(50, 100, wgrid, 1LL);
	[&]{
		for(ll y = 1; y < 50; y += 2)
		{
			for (ll x = 1; x < 99; x += 2)
	...
	ll b = 1;
	vvt<ll> bgrid; MakeVVT(50, 100, bgrid, 0LL);
	[&] {
		for(ll y = 1; y < 50; y += 2)
		{
			for (ll x = 1; x < 99; x += 2)
	...
	vvt<ll> ans; MakeVVT(100, 100, ans);
	rep(y, 0, 50) rep(x, 0, 100) ans[y][x] = wgrid[y][x];
	rep(y, 50, 100) rep(x, 0, 100) ans[y][x] = bgrid[y-50][x];
	cout << 100 << " " << 100 << endl;
	rep(y, 0, 100)
	{
		rep(x, 0, 100)

...
//0で埋める等々
void fill_example() { lint i{ 123 };
    std::cout << std::setfill('0') << std::setw(6) << i << std::endl;
}
//pair型vectorの整列。sortの第三因数に指定して用いる。
bool first_smaller(std::pair<lint, lint>& a, std::pair<lint, lint>& b) {
...
lint combination(lint a, lint b) {
    lint ans{ 1 }, ta{ std::max(a, b) }, tb{ std::min(a, b) };
    while (ta > std::abs(a - b)) { ans *= ta; --ta; }
...
int main() {
    ...
    if (K <= max && K % p == 0) cout << "POSSIBLE" << endl;

...
signed main() {
	...
	valuelist.push_back(a[0]);
	rep(i,N){
		if(t<a[i]){
			t=a[i];
			indexlist.push_back(i);
			valuelist.push_back(a[i]);
		}
	}
	...
	rep(j,N){
		ll i=N-1-j;
		ll index=lower_bound(all(valuelist),v[i].first)-valuelist.begin();
		if(index==0)continue;
		if(valuelist[index]==v[i].first && ans[indexlist[index]]==0){
			ll prevvalue=valuelist[index-1];
			ll ishi=v[i].first-prevvalue;
			ans[indexlist[index]]=(j+1)*ishi;
		}else{
			debug(index);
			debug(v[i]);
			ll prevvalue=valuelist[index-1];
			ll ishi=v[i].first-prevvalue;
			ans[indexlist[index]]+=ishi;
		}

	}
	debug(ans);
	ll sum=accumulate(all(a),0LL);
	ll anssum=accumulate(all(ans),0LL);
	ans[0]=sum-anssum;

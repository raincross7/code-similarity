...
void solve(int N, int K, vi h) {
  vi uh(all(h));
  uh.push_back(0);
  SORT(uh);
  uniq(uh);
  int M = sz(uh);
  vector dp(N + 1, vvi(K + 1, vi(M, INF)));
  rep(i, M)dp[0][0][i] = uh[i];
  rep(i, N) {
    int uhi = lower_bound(all(uh), h[i]) - uh.begin();
    rep(k, K + 1)rep(m, M){
      int cost = dp[i][k][m];
      if(h[i] > uh[m])cost += h[i] - uh[m];
      amin(dp[i + 1][k][uhi], cost);
    }
    rep(k, K)rep(m, M)amin(dp[i + 1][k + 1][m], dp[i][k][m]);
  }
  COUT(min_of(dp[N][K]));
}

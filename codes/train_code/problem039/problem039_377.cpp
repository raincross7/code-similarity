...
_int used[310][310][310];
Int mem[310][310][310];

Int dfs(Int i, Int k, Int last){
  if(i == N) return 0;
  if(used[i][k][last]++) return mem[i][k][last];

  Int a = INF, b = INF;
  a = dfs(i+1, k, i) + max(0LL, H[i] - H[last]);
  if(k < K) b = dfs(i + 1, k + 1, last);
  Int res = min(a, b);
  return mem[i][k][last] = res;
}
signed main(){

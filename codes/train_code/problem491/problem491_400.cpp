...
int main(){
   ll n;cin>>n;
   rep(j,n)rep(i,3)cin>>abc[i][j];
   CLR(dp);
   rep(i,n){
      dp[i+1][0]=max(dp[i][1]+abc[0][i],max(dp[i][2]+abc[0][i],dp[i+1][0]));
      dp[i+1][1]=max(dp[i][2]+abc[1][i],max(dp[i][0]+abc[1][i],dp[i+1][1]));
      dp[i+1][2]=max(dp[i][0]+abc[2][i],max(dp[i][1]+abc[2][i],dp[i+1][2]));
   }
   ll ans=0;

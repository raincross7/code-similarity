...
int n,m,dp[2005][2005][2][2];
vi a,b;

int f(int i,int j,bool x,bool y){
    if(i>=n || j>=m)return x&&y;
    int &ans=dp[i][j][x][y];
    if(ans!=-1)return ans;
    ans=0;
    if(a[i]==b[j])ans=(ans+f(i+1,j+1,1,1))%MOD;
    if(x && y)ans=(ans+f(i+1,j+1,1,1))%MOD;
    if(x)ans=(ans+f(i,j+1,1,0))%MOD;
    if(y)ans=(ans+f(i+1,j,0,1))%MOD;
    return ans;
}
